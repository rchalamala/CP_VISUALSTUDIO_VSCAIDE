...
template <typename T>
struct Serializer {
  static void serialize(std::ostream& out, T val) { out << val; }
...
struct Serializer<std::string> {
  ...
  static std::string deserialize(std::istream& in) {
    char c;
    in >> c;
    if (c != '"') throw std::invalid_argument("Expected a quote character");
    std::string s;
    for (;;) {
      c = in.get();
      if (c == '"') break;
      if (in.eof())
        throw std::invalid_argument(
            "End of file reached while reading a string");
      s.push_back(c);
    }
    return s;
  }
...
struct Serializer<std::vector<T> > {
  ...
  static std::vector<T> deserialize(std::istream& in) {
    char c;
    in >> c;
    if (c != '{') throw std::invalid_argument("Expected a { character");
    std::vector<T> val;
    for (;;) {
      in >> c;
      if (c == '}') break;
      ...
      if (in.eof())
        throw std::invalid_argument(
            "End of file reached while reading a vector");
