...
bool customChecker(istream& input, istream& userOutput, istream& judgeOutput,
  ...
  CAIDE_TC_RETURN_TYPE result;
  tcread(userOutput, result);
  // Declare and read judge return value
  CAIDE_TC_RETURN_TYPE judgeResult;
  tcread(judgeOutput, judgeResult);
  // Declare and read input parameters
  char caide_char;
#define CAIDE_TC_PARAM(type, name) \
  type name;                       \
  input >> caide_char;             \
  tcread(input, name);
  CAIDE_TC_PARAM_LIST
...
int main() {
  ...
  // Prepare the list of test cases in correct order; add recently created test
  // cases too.
    ...
    if (testState == "Skip") {
      ...
      try {
        runTest((testDir + testName + ".in").c_str(),
                (testDir + testName + ".out").c_str(), result);
      } catch (...) {
      ...
      if (USE_CUSTOM_CHECKER) {
        try {
          ...
          cerr << "Checker for test " << testName << " threw an exception"
               << endl;
...
// - CAIDE_TC_PARAM_LIST calls CAIDE_TC_PARAM macro for each parameter of the
// main method.
//
...
// #define CAIDE_TC_PARAM_LIST  CAIDE_TC_PARAM(string, s) CAIDE_TC_PARAM(string,
// t)

// Note: we cannot forward declare the actual class defined by Topcoder
// because we don't know what methods/members it will contain after it's
// implemented. So we declare a wrapper function around the solution class. Its
// definition is in solution file and must not be modified! It will be removed
// before submission.
#define CAIDE_TC_PARAM(type, name) type name,
CAIDE_TC_RETURN_TYPE solve(CAIDE_TC_PARAM_LIST int);
#undef CAIDE_TC_PARAM
...
#define CAIDE_TC_PARAM(type, name) \
  type name;                       \
  caide_in >> caide_char;          \
  tcread(caide_in, name);
  CAIDE_TC_PARAM_LIST
#undef CAIDE_TC_PARAM

  // Run solution
#define CAIDE_TC_PARAM(type, name) name,
  CAIDE_TC_RETURN_TYPE caide_result = solve(CAIDE_TC_PARAM_LIST 0);
#undef CAIDE_TC_PARAM
