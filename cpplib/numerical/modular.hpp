...
public:
	...
	template<typename T1 = T> typename std::enable_if<(sizeof(typename Modular<T1>::Type) >= 8), Modular&>::type& operator*=(const Modular& rhs)
	{
		...
		auto quotient = static_cast<typename Modular<T1>::Type>(static_cast<long double>(value) * rhs.value / modulus());
...
template<typename T1, typename T2> Modular<T1> operator-(Modular<T1> lhs, const T2& rhs) { return lhs -= rhs; }

template<typename T1, typename T2> Modular<T1> operator-(const T2& lhs, Modular<T1> rhs) { return rhs -= lhs; }
