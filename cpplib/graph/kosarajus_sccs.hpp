...
template <class Edge>
std::vector<std::vector<std::size_t>> kosarajus_sccs(const Graph<Edge>& graph) {
  ...
  std::function<void(std::size_t)> dfs = [&](const std::size_t& at) -> void {
    ...
    for (const auto& neighbor : graph[at])
      if (!visited[neighbor.to]) dfs(neighbor.to);
    order.push(at);
  };
  ...
  std::vector<std::vector<std::size_t>> sCCs;
  std::function<void(std::size_t)> transposedDFS =
      [&](const std::size_t& at) -> void {
    visited[at] = false;
    sCCs.back().push_back(at);
    for (const auto& neighbor : graph[at])
      if (visited[neighbor.to]) transposedDFS(neighbor.to);
  };
  while (!order.empty()) {
