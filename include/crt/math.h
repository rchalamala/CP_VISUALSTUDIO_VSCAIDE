...
#define PLOSS _PLOSS
    ...
    defined(_GNU_SOURCE) || defined(_BSD_SOURCE) || defined(_USE_MATH_DEFINES)
...
struct __mingw_types_compatible_p {
  static const bool result = false;
};

template <typename type1>
struct __mingw_types_compatible_p<type1, type1> {
  static const bool result = true;
};

template <typename type1>
struct __mingw_types_compatible_p<const type1, type1> {
  static const bool result = true;
};

template <typename type1>
struct __mingw_types_compatible_p<type1, const type1> {
  static const bool result = true;
};
}

#define __mingw_types_compatible_p(type1, type2) \
  __mingw_types_compatible_p<type1, type2>::result
#else
#define __mingw_types_compatible_p(type1, type2) \
...
typedef union __mingw_dbl_type_t {
  ...
  __C89_NAMELESS struct { unsigned int low, high; } lh;
} __mingw_dbl_type_t;
...
double __cdecl _cabs(
    struct _complex _ComplexA); /* Overridden to use our cabs.  */
...
__CRT_INLINE int __cdecl __fpclassifyl(long double x) {
  ...
  if (!e) {
    ...
    return (((hlp.lh.high & 0x7fffffff) | hlp.lh.low) == 0 ? FP_INFINITE
                                                           : FP_NAN);
  return FP_NORMAL;
...
__CRT_INLINE int __cdecl __fpclassify(double x) {
  ...
  h &= 0x7ff00000;
  if ((h | l) == 0) return FP_ZERO;
  if (!h) return FP_SUBNORMAL;
  if (h == 0x7ff00000) return (l ? FP_NAN : FP_INFINITE);
  return FP_NORMAL;
#elif defined(__i386__) || defined(_X86_)
  unsigned short sw;
  __asm__ __volatile__("fxam; fstsw %%ax;" : "=a"(sw) : "t"(x));
...
__CRT_INLINE int __cdecl __fpclassifyf(float x) {
  ...
  hlp.x = x;
  hlp.val &= 0x7fffffff;
  if (hlp.val == 0) return FP_ZERO;
  if (hlp.val < 0x800000) return FP_SUBNORMAL;
...
#define __dfp_expansion(__call, __fin, x)                                    \
  __mingw_choose_expr(                                                       \
      __mingw_types_compatible_p(__typeof__(x), _Decimal32), __call##d32(x), \
      __mingw_choose_expr(                                                   \
...
#define fpclassify(x)                                                         \
  __mingw_choose_expr(                                                        \
      __mingw_types_compatible_p(__typeof__(x), double), __fpclassify(x),     \
      __mingw_choose_expr(                                                    \
          __mingw_types_compatible_p(__typeof__(x), float), __fpclassifyf(x), \
          __mingw_choose_expr(                                                \
...
#define isnan(x)                                                         \
  __mingw_choose_expr(                                                   \
      __mingw_types_compatible_p(__typeof__(x), double), __isnan(x),     \
      __mingw_choose_expr(                                               \
          __mingw_types_compatible_p(__typeof__(x), float), __isnanf(x), \
          __mingw_choose_expr(                                           \
              __mingw_types_compatible_p(__typeof__(x), long double),    \
              __isnanl(x),                                               \
...
#define signbit(x)                                                         \
  __mingw_choose_expr(                                                     \
      __mingw_types_compatible_p(__typeof__(x), double), __signbit(x),     \
      __mingw_choose_expr(                                                 \
          __mingw_types_compatible_p(__typeof__(x), float), __signbitf(x), \
          __mingw_choose_expr(                                             \
              __mingw_types_compatible_p(__typeof__(x), long double),      \
              __signbitl(x),                                               \
...
__CRT_INLINE float __cdecl ldexpf(float x, int expn) {
  return (float)ldexp((double)x, expn);
}
#endif
...
#ifndef __CRT__NO_INLINE
__CRT_INLINE __cdecl __isnand32(_Decimal32 x) { return __builtin_isnand32(x); }

__CRT_INLINE __cdecl __isnand64(_Decimal64 x) { return __builtin_isnand64(x); }
