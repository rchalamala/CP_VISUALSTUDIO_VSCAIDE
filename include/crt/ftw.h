...
#define FTW_DEPTH 8
/* Tell callback to return FTW_* values instead of zero to continue and non-zero
 * to terminate.  */
...
/* Continue with FTW_DP callback for current directory (if FTW_DEPTH) and then
 * its siblings.  */
...
int nftw64(const char *,
           int (*)(const char *, const struct stat64 *, int, struct FTW *), int,
           int);
