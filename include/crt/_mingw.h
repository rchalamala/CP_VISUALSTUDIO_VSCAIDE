...
#define __MINGW_BROKEN_INTERFACE(x) \
  __MINGW_PRAGMA_PARAM(             \
      message("Interface " _CRT_STRINGIZE(x) " has unverified layout."))
...
#define __int64 long long
    ...
    !defined(                                                                  \
        __SIZEOF_INT128__) /* clang >= 3.1 has __int128 but no size macro */
...
/* MSVC defines _NATIVE_NULLPTR_SUPPORTED when nullptr is supported. We emulate
 * it here for GCC. */
...
#define __USE_MINGW_ANSI_STDIO \
  1 /* was defined as nonzero or empty so it should be 1 */
#else
#define __USE_MINGW_ANSI_STDIO \
  0 /* was defined as (int)zero and non-empty so it should be 0 */
#endif
...
#define __CRT_UUID_DECL(type, l, w1, w2, b1, b2, b3, b4, b5, b6, b7, b8) \
  extern "C++" {                                                         \
  template <>                                                            \
  struct __mingw_uuidof_s<type> {                                        \
    static constexpr IID __uuid_inst = {                                 \
        l, w1, w2, {b1, b2, b3, b4, b5, b6, b7, b8}};                    \
  };                                                                     \
  template <>                                                            \
  constexpr const GUID &__mingw_uuidof<type>() {                         \
