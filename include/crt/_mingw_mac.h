...
#define __MINGW64_VERSION_STR                                               \
  ...
  "." __MINGW64_STRINGIFY(__MINGW64_VERSION_MINOR) "." __MINGW64_STRINGIFY( \
...
#define __MINGW_MSVC2005_DEPREC_STR                                      \
  ...
  "_CRT_NONSTDC_NO_DEPRECATE to disable deprecation"
...
#define __mingw_bos_ptr_chk(p, n, maxtype)      \
  __mingw_bos_cond_chk(!__mingw_bos_known(p) || \
                       __mingw_bos(p, maxtype) >= (size_t)(n))
#define __mingw_bos_ptr_chk_warn(p, n, maxtype)         \
  (__mingw_bos_known(p) && __builtin_constant_p((n)) && \
           __mingw_bos(p, maxtype) < (size_t)(n)        \
       ? __mingw_chk_fail_warn()                        \
       : __mingw_bos_ptr_chk(p, n, maxtype))
#define __mingw_bos_ovr \
  __mingw_ovr __attribute__((__always_inline__)) __mingw_attribute_artificial
#define __mingw_bos_extern_ovr                                                 \
  extern __inline__ __cdecl __attribute__((__always_inline__, __gnu_inline__)) \
...
/* If _FORTIFY_SOURCE is enabled, some inline functions may use
   ...
   of such a function is used.  Set _FORTIFY_VA_ARG=0 in this case.  */
    ...
    !defined(__clang__) && (!defined(_FORTIFY_VA_ARG) || _FORTIFY_VA_ARG > 0)
