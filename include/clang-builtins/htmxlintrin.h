...
extern __inline long
  ...
  if (__builtin_expect(__builtin_tbegin(0), 1)) return _HTM_TBEGIN_STARTED;
  return 0;
}
...
extern __inline long
    __attribute__((__gnu_inline__, __always_inline__, __artificial__))
    __TM_end(void) {
  if (__builtin_expect(__builtin_tend(0), 1)) return 1;
  return 0;
}
...
extern __inline long
  ...
  if (_HTM_STATE(__builtin_ttest()) == _HTM_NONTRANSACTIONAL) {
    ...
    if (!_TEXASR_FAILURE_SUMMARY(texasrl)) texasrl = 0;
  } else
