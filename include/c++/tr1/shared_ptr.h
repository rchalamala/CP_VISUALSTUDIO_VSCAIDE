...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  class bad_weak_ptr : public std::exception {
   public:
    virtual char const* what() const throw() { return "tr1::bad_weak_ptr"; }
  };
  ...
  class __shared_count {
   public:
    ...
    explicit __shared_count(std::auto_ptr<_Tp>& __r)
        : _M_pi(new _Sp_counted_base_impl<_Tp*, _Sp_deleter<_Tp>, _Lp>(
  ...
  class __weak_count {
   public:
    ...
    __weak_count(const __shared_count<_Lp>& __r)
        : _M_pi(__r._M_pi)  // nothrow
    {
      if (_M_pi != 0) _M_pi->_M_weak_add_ref();
    }
    ...
    {
      if (_M_pi != 0) _M_pi->_M_weak_release();
    }

    __weak_count<_Lp>& operator=(const __shared_count<_Lp>& __r)  // nothrow
    {
      _Sp_counted_base<_Lp>* __tmp = __r._M_pi;
      if (__tmp != 0) __tmp->_M_weak_add_ref();
      if (_M_pi != 0) _M_pi->_M_weak_release();
      _M_pi = __tmp;
      return *this;
    }
    ...
    friend inline bool operator==(const __weak_count<_Lp>& __a,
  ...
  class __shared_ptr {
   public:
    ...
    explicit __shared_ptr(std::auto_ptr<_Tp1>& __r)
        : _M_ptr(__r.get()),
          _M_refcount() {  // TODO requries delete __r.release() well-formed
      __glibcxx_function_requires(_ConvertibleConcept<_Tp1*, _Tp*>) typedef int
          _IsComplete[sizeof(_Tp1)];
      _Tp1* __tmp = __r.get();
    ...
    typename std::tr1::add_reference<_Tp>::type operator*()
        const  // never throws
    {
   ...
   private:
    ...
    template <typename _Tp1>
    friend inline bool operator==(const __shared_ptr& __a,
  ...
  template <typename _Tp, _Lock_policy _Lp>
  inline void swap(__shared_ptr<_Tp, _Lp>& __a, __shared_ptr<_Tp, _Lp>& __b) {
    __a.swap(__b);
  }
  ...
  inline _Del* get_deleter(const __shared_ptr<_Tp, _Lp>& __p) {
#if __cpp_rtti
  ...
  class __weak_ptr {
   public:
    ...
    __weak_ptr(const __weak_ptr<_Tp1, _Lp>& __r)
        : _M_refcount(__r._M_refcount)  // never throws
    {
      __glibcxx_function_requires(_ConvertibleConcept<_Tp1*, _Tp*>) _M_ptr =
          __r.lock().get();
    }
    ...
    __shared_ptr<_Tp, _Lp> lock() const  // never throws
    {
      ...
      if (expired()) return __shared_ptr<element_type, _Lp>();
   ...
   private:
    ...
    template <typename _Tp1>
    friend inline bool operator<(const __weak_ptr& __lhs,
  ...
  template <typename _Tp, _Lock_policy _Lp>
  inline void swap(__weak_ptr<_Tp, _Lp>& __a, __weak_ptr<_Tp, _Lp>& __b) {
    __a.swap(__b);
  }
  ...
  class __enable_shared_from_this {
   ...
   private:
    ...
    template <typename _Tp1>
    friend void __enable_shared_from_this_helper(
        const __shared_count<_Lp>& __pn, const __enable_shared_from_this* __pe,
        const _Tp1* __px) {
      if (__pe != 0) __pe->_M_weak_assign(const_cast<_Tp1*>(__px), __pn);
    }
  ...
  inline shared_ptr<_Tp> static_pointer_cast(const shared_ptr<_Tp1>& __r) {
    return shared_ptr<_Tp>(__r, __static_cast_tag());
  }

  template <typename _Tp, typename _Tp1>
  inline shared_ptr<_Tp> const_pointer_cast(const shared_ptr<_Tp1>& __r) {
    return shared_ptr<_Tp>(__r, __const_cast_tag());
  }

  template <typename _Tp, typename _Tp1>
  inline shared_ptr<_Tp> dynamic_pointer_cast(const shared_ptr<_Tp1>& __r) {
  ...
  class weak_ptr : public __weak_ptr<_Tp> {
   public:
    ...
    {
#ifdef __GTHREADS
      if (this->expired()) return shared_ptr<_Tp>();
  ...
  class enable_shared_from_this {
   ...
   private:
    ...
    template <typename _Tp1>
    friend void __enable_shared_from_this_helper(
        const __shared_count<>& __pn, const enable_shared_from_this* __pe,
        const _Tp1* __px) {
      if (__pe != 0) __pe->_M_weak_assign(const_cast<_Tp1*>(__px), __pn);
    }
