...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  struct hash<_Tp*> : public std::unary_function<_Tp*, size_t> {
    size_t operator()(_Tp* __p) const { return reinterpret_cast<size_t>(__p); }
  };

  /// Explicit specializations for integer types.
#define _TR1_hashtable_define_trivial_hash(_Tp)          \
  template <>                                            \
  inline size_t hash<_Tp>::operator()(_Tp __val) const { \
...
#undef _TR1_hashtable_define_trivial_hash
  ...
  struct _Fnv_hash_base {
    ...
    static size_t hash(const _Tp* __ptr, size_t __clength) {
      ...
      for (; __clength; --__clength) __result = (__result * 131) + *__cptr++;
      return __result;
    }
  };
  ...
  template <>
  inline size_t hash<float>::operator()(float __val) const {
  ...
  template <>
  inline size_t hash<double>::operator()(double __val) const {
    // 0 and -0 both hash to zero.
  ...
  template <>
  _GLIBCXX_PURE size_t hash<long double>::operator()(long double __val) const;

  /// Explicit specialization of member operator for non-builtin types.
  template <>
  _GLIBCXX_PURE size_t hash<string>::operator()(string) const;

  template <>
  _GLIBCXX_PURE size_t hash<const string&>::operator()(const string&) const;

#ifdef _GLIBCXX_USE_WCHAR_T
  template <>
  _GLIBCXX_PURE size_t hash<wstring>::operator()(wstring) const;

  template <>
  _GLIBCXX_PURE size_t hash<const wstring&>::operator()(const wstring&) const;
#endif
  }  // namespace tr1
