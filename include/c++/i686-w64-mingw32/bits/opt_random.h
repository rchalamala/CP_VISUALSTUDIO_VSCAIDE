...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  void normal_distribution<double>::__generate(
      typename normal_distribution<double>::result_type * __f,
    ...
    while (__f + 1 < __t) {
      ...
      do {
        ...
        if (__urngrange > __maskval) {
          if (__detail::_Power_of_2(__uerngrange))
            __v.__i = _mm_and_si128(_mm_set_epi64x(__urng(), __urng()), __mask);
          else {
            const __uctype __uerange = __maskval + 1;
            const __uctype __scaling = __urngrange / __uerange;
            const __uctype __past = __uerange * __scaling;
            uint64_t __v1;
            do __v1 = __uctype(__urng()) - __urngmin;
            while (__v1 >= __past);
            __v1 /= __scaling;
            uint64_t __v2;
            do __v2 = __uctype(__urng()) - __urngmin;
            while (__v2 >= __past);
            __v2 /= __scaling;
          ...
          __v.__i = _mm_and_si128(_mm_set_epi64x(__v1, __v2), __mask);
        } else {
          size_t __nrng = 2;
          ...
          uint64_t __v1;
          do {
            do __tmp = __uctype(__urng()) - __urngmin;
            while (__tmp >= __past);
      ...
      double __mult =
          (std::sqrt(-2.0 * std::log(__le) / __le) * __param.stddev());
