...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  class file_status {
   public:
    ...
    friend bool operator==(const file_status&,
                           const file_status&) noexcept = default;
#endif
  ...
  class directory_entry {
   public:
    ...
    bool is_block_file() const { return _M_file_type() == file_type::block; }
    ...
    bool is_directory() const { return _M_file_type() == file_type::directory; }

    bool is_directory(error_code& __ec) const noexcept {
      return _M_file_type(__ec) == file_type::directory;
    }

    bool is_fifo() const { return _M_file_type() == file_type::fifo; }
    ...
    bool is_socket() const { return _M_file_type() == file_type::socket; }
    ...
    bool is_symlink() const {
      if (_M_type != file_type::none) return _M_type == file_type::symlink;
    ...
    uintmax_t file_size() const { return filesystem::file_size(_M_path); }
    ...
    file_status status() const { return filesystem::status(_M_path); }
   ...
   private:
    ...
    file_type _M_file_type(error_code& __ec) const noexcept {
      if (_M_type != file_type::none && _M_type != file_type::symlink) {
        __ec.clear();
        return _M_type;
      }
  ...
  struct __directory_iterator_proxy {
   ...
   private:
    ...
    explicit __directory_iterator_proxy(const directory_entry& __e)
        : _M_entry(__e) {}
  ...
  class directory_iterator {
   public:
    ...
    directory_iterator& operator=(directory_iterator&& __rhs) noexcept =
        default;
  ...
  class recursive_directory_iterator {
   ...
   private:
    ...
    friend bool operator==(const recursive_directory_iterator& __lhs,
                           const recursive_directory_iterator& __rhs) noexcept {
