...
namespace std _GLIBCXX_VISIBILITY(default) {
...
#if __cplusplus >= 201103L
    _List_node_header(_List_node_header&& __x) noexcept : _List_node_base {
  ...
  class _List_base {
...
#if !_GLIBCXX_INLINE_VERSION
    size_t _M_distance(const __detail::_List_node_base* __first,
   ...
   public:
    ...
    void _M_init() _GLIBCXX_NOEXCEPT { this->_M_impl._M_node._M_init(); }
  };
  ...
  class list : protected _List_base<_Tp, _Alloc> {
   ...
   protected:
    ...
    // return the stored size
    size_t _M_node_count() const { return this->_M_get_size(); }
#else
    // dummy implementations used when the size is not stored
    static size_t _S_distance(const_iterator, const_iterator) { return 0; }

    // count the number of nodes
    size_t _M_node_count() const { return std::distance(begin(), end()); }
#endif
   ...
   public:
    list(list&& __x, const allocator_type& __a) noexcept(
     ...
     */
    list& operator=(list&& __x) noexcept(
    ...
    size_type size() const _GLIBCXX_NOEXCEPT { return _M_node_count(); }
...
#endif
    {
      ...
      if (this != std::__addressof(__x)) _M_check_equal_allocators(__x);
...
#endif
    {
      if (__first != __last) {
        ...
        this->_M_transfer(__position._M_const_cast(), __first._M_const_cast(),
...
#undef _GLIBCXX_LIST_REMOVE_RETURN_TYPE_TAG
   ...
   protected:
    ...
    void _M_check_equal_allocators(list& __x) _GLIBCXX_NOEXCEPT {
      if (std::__alloc_neq<typename _Base::_Node_alloc_type>::_S_do_it(
    ...
    void _M_move_assign(list&& __x, false_type) {
      if (__x._M_get_Node_allocator() == this->_M_get_Node_allocator())
        ...
        _M_assign_dispatch(std::make_move_iterator(__x.begin()),
                           std::make_move_iterator(__x.end()), __false_type{});
    }
#endif
  };
  ...
  template <typename _Tp>
  inline ptrdiff_t __distance(_GLIBCXX_STD_C::_List_iterator<_Tp> __first,
  ...
  template <typename _Tp>
  inline ptrdiff_t __distance(_GLIBCXX_STD_C::_List_const_iterator<_Tp> __first,
