...
#define __cpp_lib_ranges 201911L
  ...
  concept __eq_builtin_ptr_cmp = requires(_Tp&& __t, _Up&& __u) {
    { __t == __u }
    ->same_as<bool>;
  }
  ...
  // BUILTIN-PTR-CMP(T, <, U)
  template <typename _Tp, typename _Up>
  concept __less_builtin_ptr_cmp = requires(_Tp&& __t, _Up&& __u) {
    { __t < __u }
    ->same_as<bool>;
  }
  &&convertible_to<_Tp, const volatile void*>&&
  ...
  struct equal_to {
    template <typename _Tp, typename _Up>
        ...
        __detail::__eq_builtin_ptr_cmp<_Tp, _Up> constexpr bool operator()(
  ...
  struct less {
    template <typename _Tp, typename _Up>
        ...
        __detail::__less_builtin_ptr_cmp<_Tp, _Up> constexpr bool operator()(
            _Tp&& __t, _Up&& __u) const
        noexcept(noexcept(std::declval<_Tp>() < std::declval<_Up>())) {
      if constexpr (__detail::__less_builtin_ptr_cmp<_Tp, _Up>) {
#ifdef __cpp_lib_is_constant_evaluated
        if (std::is_constant_evaluated()) return __t < __u;
#endif
  ...
  struct greater {
    template <typename _Tp, typename _Up>
        requires totally_ordered_with<_Tp, _Up> ||
        __detail::__less_builtin_ptr_cmp<_Up, _Tp> constexpr bool operator()(
            _Tp&& __t, _Up&& __u) const
        noexcept(noexcept(std::declval<_Up>() < std::declval<_Tp>())) {
