...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  template <typename _BiIter, typename _Alloc, typename _CharT,
            typename _TraitsT, _RegexExecutorPolicy __policy, bool __match_mode>
  bool __regex_algo_impl(_BiIter __s, _BiIter __e,
  ...
  struct regex_traits {
   ...
   public:
     ...
     */
    template <typename _Fwd_iter>
    char_class_type lookup_classname(_Fwd_iter __first, _Fwd_iter __last,
     ...
     * object.
     */
    locale_type getloc() const { return _M_locale; }
  ...
  class basic_regex {
   public:
     ...
     * character sequence.
     */
    basic_regex() : _M_flags(ECMAScript), _M_loc(), _M_automaton(nullptr) {}
     ...
     */
    explicit basic_regex(const _Ch_type* __p, flag_type __f = ECMAScript)
    ...
    basic_regex(_FwdIter __first, _FwdIter __last, flag_type __f = ECMAScript)
        : basic_regex(std::move(__first), std::move(__last), locale_type(),
                      __f) {}
     ...
     *        containing a regular expression.
     */
    basic_regex& operator=(const _Ch_type* __p) { return this->assign(__p); }
    ...
    basic_regex& assign(const _Ch_type* __p, size_t __len,
     ...
     */
    template <typename _Ch_traits, typename _Alloc>
    basic_regex& assign(const basic_string<_Ch_type, _Ch_traits, _Alloc>& __s,
                        flag_type __flags = ECMAScript) {
      return this->assign(
          basic_regex(__s.data(), __s.data() + __s.size(), _M_loc, __flags));
    }
     ...
     * unchanged.
     */
    basic_regex& assign(initializer_list<_Ch_type> __l,
                        flag_type __flags = ECMAScript) {
    ...
    unsigned int mark_count() const {
      if (_M_automaton) return _M_automaton->_M_sub_count() - 1;
      return 0;
    }
     ...
     */
    flag_type flags() const { return _M_flags; }
     ...
     *        object.
     */
    locale_type getloc() const { return _M_loc; }
    ...
    void _M_dot(std::ostream& __ostr) { _M_automaton->_M_dot(__ostr); }
#endif
   ...
   private:
    ...
    friend bool __detail::__regex_algo_impl(_Bp, _Bp, match_results<_Bp, _Ap>&,
  ...
  constexpr regex_constants::syntax_option_type basic_regex<_Ch, _Tr>::icase;

  template <typename _Ch, typename _Tr>
  constexpr regex_constants::syntax_option_type basic_regex<_Ch, _Tr>::nosubs;

  template <typename _Ch, typename _Tr>
  constexpr regex_constants::syntax_option_type basic_regex<_Ch, _Tr>::optimize;

  template <typename _Ch, typename _Tr>
  constexpr regex_constants::syntax_option_type basic_regex<_Ch, _Tr>::collate;
  ...
  template <typename _Ch, typename _Tr>
  constexpr regex_constants::syntax_option_type basic_regex<_Ch, _Tr>::basic;

  template <typename _Ch, typename _Tr>
  constexpr regex_constants::syntax_option_type basic_regex<_Ch, _Tr>::extended;

  template <typename _Ch, typename _Tr>
  constexpr regex_constants::syntax_option_type basic_regex<_Ch, _Tr>::awk;

  template <typename _Ch, typename _Tr>
  constexpr regex_constants::syntax_option_type basic_regex<_Ch, _Tr>::grep;

  template <typename _Ch, typename _Tr>
  constexpr regex_constants::syntax_option_type basic_regex<_Ch, _Tr>::egrep;
#endif  // ! C++17
  ...
  class sub_match : public std::pair<_BiIter, _BiIter> {
   ...
   public:
    ...
    string_type str() const {
      return this->matched ? string_type(this->first, this->second)
                           : string_type();
    }
   ...
   private:
    ...
    __enable_if_t<__detail::__is_contiguous_iter<_Iter>::value, __string_view>
    ...
    template <typename _Iter = _BiIter>
    __enable_if_t<!__detail::__is_contiguous_iter<_Iter>::value, string_type>
    _M_str() const {
      return str();
    }
  };
   ...
   */
  template <typename _BiIter>
  inline bool operator==(const sub_match<_BiIter>& __lhs,
  ...
  inline auto operator<=>(
      ...
      const sub_match<_BiIter>&
          __rhs) noexcept(__detail::__is_contiguous_iter<_BiIter>::value) {
  ...
  using __sub_match_string =
      basic_string<typename iterator_traits<_Bi_iter>::value_type, _Ch_traits,
                   _Ch_alloc>;
  /// @endcond
  ...
  inline auto operator<=>(
      const sub_match<_Bi_iter>& __lhs,
      const __sub_match_string<_Bi_iter, _Ch_traits, _Alloc>&
  ...
  inline basic_ostream<_Ch_type, _Ch_traits>& operator<<(
  ...
  class match_results : private std::vector<sub_match<_Bi_iter>, _Alloc> {
   ...
   public:
     ...
     * @post size() returns 0 and str() returns an empty string.
     */
    explicit match_results(const _Alloc& __a) noexcept : _Base_type(__a) {}
    ...
    size_type max_size() const noexcept { return _Base_type::max_size() - 3; }
    ...
    const_reference operator[](size_type __sub) const {
      ...
      return __sub < size() ? _Base_type::operator[](__sub)
                            : _M_unmatched_sub();
    }
     ...
     */
    const_iterator begin() const noexcept { return _Base_type::begin(); }
    ...
    _Out_iter format(
        _Out_iter __out, const char_type* __fmt_first,
    ...
    _Out_iter format(
        _Out_iter __out, const basic_string<char_type, _St, _Sa>& __fmt,
        match_flag_type __flags = regex_constants::format_default) const {
    ...
    string_type format(
      ...
      format(std::back_inserter(__result), __fmt,
  ...
  inline bool operator==(const match_results<_Bi_iter, _Alloc>& __m1,
                         const match_results<_Bi_iter, _Alloc>& __m2) noexcept {
    if (__m1.ready() != __m2.ready()) return false;
    if (!__m1.ready())  // both are not ready
      return true;
    if (__m1.empty() != __m2.empty()) return false;
    if (__m1.empty())  // both are empty
      return true;
    return __m1.prefix() == __m2.prefix() && __m1.size() == __m2.size() &&
   ...
   */
  template <typename _Bi_iter, typename _Alloc>
  inline void swap(match_results<_Bi_iter, _Alloc> & __lhs,
  ...
  inline bool regex_match(_Bi_iter __first, _Bi_iter __last,
                          const basic_regex<_Ch_type, _Rx_traits>& __re,
                          regex_constants::match_flag_type __flags =
                              regex_constants::match_default) {
  ...
  inline bool regex_match(
      const _Ch_type* __s, match_results<const _Ch_type*, _Alloc>& __m,
      const basic_regex<_Ch_type, _Rx_traits>& __re,
  ...
  inline bool regex_match(
      const _Ch_type* __s, const basic_regex<_Ch_type, _Rx_traits>& __re,
      regex_constants::match_flag_type __f = regex_constants::match_default) {
   ...
   * @throws an exception of type regex_error.
   */
  template <typename _Ch_traits, typename _Str_allocator, typename _Ch_type,
            typename _Rx_traits>
  inline bool regex_match(
  ...
  template <typename _Bi_iter, typename _Alloc, typename _Ch_type,
            typename _Rx_traits>
  inline bool regex_search(_Bi_iter __s, _Bi_iter __e,
                           match_results<_Bi_iter, _Alloc> & __m,
                           const basic_regex<_Ch_type, _Rx_traits>& __re,
                           regex_constants::match_flag_type __flags =
                               regex_constants::match_default) {
   ...
   */
  template <typename _Bi_iter, typename _Ch_type, typename _Rx_traits>
  inline bool regex_search(_Bi_iter __first, _Bi_iter __last,
                           const basic_regex<_Ch_type, _Rx_traits>& __re,
                           regex_constants::match_flag_type __flags =
                               regex_constants::match_default) {
    match_results<_Bi_iter> __what;
  ...
  template <typename _Ch_type, typename _Rx_traits>
  inline bool regex_search(
      const _Ch_type* __s, const basic_regex<_Ch_type, _Rx_traits>& __e,
      regex_constants::match_flag_type __f = regex_constants::match_default) {
   ...
   * @throws an exception of type regex_error.
   */
  template <typename _Ch_traits, typename _String_allocator, typename _Ch_type,
            typename _Rx_traits>
  inline bool regex_search(
  ...
  template <typename _Out_iter, typename _Bi_iter, typename _Rx_traits,
            typename _Ch_type, typename _St, typename _Sa>
  inline _Out_iter regex_replace(_Out_iter __out, _Bi_iter __first,
                                 _Bi_iter __last,
                                 const basic_regex<_Ch_type, _Rx_traits>& __e,
  ...
  template <typename _Rx_traits, typename _Ch_type, typename _St, typename _Sa,
            typename _Fst, typename _Fsa>
  inline basic_string<_Ch_type, _St, _Sa> regex_replace(
    ...
    regex_replace(std::back_inserter(__result), __s.begin(), __s.end(), __e,
                  __fmt, __flags);
    return __result;
  }
  ...
  inline basic_string<_Ch_type, _St, _Sa> regex_replace(
      const basic_string<_Ch_type, _St, _Sa>& __s,
      const basic_regex<_Ch_type, _Rx_traits>& __e, const _Ch_type* __fmt,
      regex_constants::match_flag_type __flags =
          regex_constants::match_default) {
    basic_string<_Ch_type, _St, _Sa> __result;
    regex_replace(std::back_inserter(__result), __s.begin(), __s.end(), __e,
   ...
   */
  template <typename _Rx_traits, typename _Ch_type, typename _St, typename _Sa>
  inline basic_string<_Ch_type> regex_replace(
      const _Ch_type* __s, const basic_regex<_Ch_type, _Rx_traits>& __e,
      const basic_string<_Ch_type, _St, _Sa>& __fmt,
      regex_constants::match_flag_type __flags =
          regex_constants::match_default) {
    ...
    regex_replace(std::back_inserter(__result), __s,
                  __s + char_traits<_Ch_type>::length(__s), __e, __fmt,
                  __flags);
    return __result;
  }
  ...
  class regex_iterator {
   public:
     ...
     */
    regex_iterator(
        _Bi_iter __a, _Bi_iter __b, const regex_type& __re,
     ...
     */
    const value_type& operator*() const noexcept { return _M_match; }
  ...
  class regex_token_iterator {
   ...
   public:
     ...
     */
    regex_token_iterator(
        _Bi_iter __a, _Bi_iter __b, const regex_type& __re, int __submatch = 0,
        regex_constants::match_flag_type __m = regex_constants::match_default)
    ...
    regex_token_iterator(
        ...
        : _M_position(__a, __b, __re, __m),
          _M_subs(__submatches, __submatches + _Nm),
          _M_n(0) {
      _M_init(__a, __b);
    }
     ...
     */
    const value_type& operator*() const { return *_M_result; }
   ...
   private:
    ...
    constexpr bool _M_end_of_seq() const { return _M_result == nullptr; }
