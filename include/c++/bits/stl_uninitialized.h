...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  inline _ForwardIterator uninitialized_copy(_InputIterator __first,
    ...
    static_assert(
        is_constructible<_ValueType2, decltype(*__first)>::value,
...
#define __cpp_lib_raw_memory_algorithms 201606L
  ...
  inline _ForwardIterator uninitialized_move(_InputIterator __first,
                                             _InputIterator __last,
                                             _ForwardIterator __result) {
    return std::uninitialized_copy(_GLIBCXX_MAKE_MOVE_ITERATOR(__first),
                                   _GLIBCXX_MAKE_MOVE_ITERATOR(__last),
                                   __result);
  }
  ...
  inline pair<_InputIterator, _ForwardIterator> uninitialized_move_n(
    ...
    auto __res = std::__uninitialized_copy_n_pair(
        _GLIBCXX_MAKE_MOVE_ITERATOR(__first), __count, __result);
  ...
  inline void
  __relocate_object_a(_Tp * __restrict __dest, _Up * __restrict __orig, _Allocator & __alloc) noexcept(
      noexcept(std::allocator_traits<_Allocator>::construct(
          __alloc, __dest,
          std::move(*__orig)))&& noexcept(std::allocator_traits<_Allocator>::
                                              destroy(__alloc, std::__addressof(
                                                                   *__orig)))) {
  ...
  inline _ForwardIterator __relocate_a(
      ...
      _Allocator &
          __alloc) noexcept(noexcept(__relocate_a_1(std::__niter_base(__first),
                                                    ...
                                                    __alloc))) {
    return __relocate_a_1(std::__niter_base(__first), std::__niter_base(__last),
