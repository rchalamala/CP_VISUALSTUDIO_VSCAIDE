...
#pragma GCC diagnostic pop
  ...
  inline __shared_count<_Lp>::__shared_count(const __weak_count<_Lp>& __r,
      ...
      : _M_pi(__r._M_pi) {
    if (_M_pi != nullptr)
      if (!_M_pi->_M_add_ref_lock_nothrow()) _M_pi = nullptr;
  }
...
#define __cpp_lib_shared_ptr_arrays 201611L
  ...
  class __shared_ptr_access<_Tp, _Lp, true, false> {
   public:
    ...
    [[__deprecated__(
        "shared_ptr<T[]>::operator* is absent from C++17")]] element_type&
  ...
  class __shared_ptr : public __shared_ptr_access<_Tp, _Lp> {
   ...
   private:
    ...
    using _UniqCompatible =
        typename enable_if<__and_<__sp_compatible_with<_Yp*, _Tp*>,
   ...
   protected:
    // If an exception is thrown this constructor has no effect.
    template <
        typename _Tp1, typename _Del,
        typename enable_if<
            __and_<__not_<is_array<_Tp>>, is_array<_Tp1>,
                   is_convertible<typename unique_ptr<_Tp1, _Del>::pointer,
                                  _Tp*>>::value,
            bool>::type = true>
