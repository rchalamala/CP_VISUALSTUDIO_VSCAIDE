...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  class map {
   ...
   private:
    ...
    __glibcxx_class_requires(_Tp, _SGIAssignableConcept)
        ...
        __glibcxx_class_requires4(_Compare, bool, _Key, _Key,
            ...
            __glibcxx_class_requires2(value_type, _Alloc_value_type,
                                      _SameTypeConcept)
#endif
                ...
                static_assert(
                    is_same<typename _Alloc::value_type, value_type>::value,
   ...
   public:
     ...
     *  keys.
     */
    const_iterator cbegin() const noexcept { return _M_t.begin(); }
     ...
     *  the keys.
     */
    const_iterator cend() const noexcept { return _M_t.end(); }
     ...
     *  according to the keys.
     */
    const_reverse_iterator crbegin() const noexcept { return _M_t.rbegin(); }
     ...
     *  order according to the keys.
     */
    const_reverse_iterator crend() const noexcept { return _M_t.rend(); }
#endif
    ...
    /// Extract a node.
    node_type extract(const key_type& __x) { return _M_t.extract(__x); }
...
#define __cpp_lib_map_try_emplace 201411
    ...
    pair<iterator, bool> try_emplace(const key_type& __k, _Args&&... __args) {
      ...
      if (__i == end() || key_comp()(__k, (*__i).first)) {
        __i = emplace_hint(
            __i, std::piecewise_construct, std::forward_as_tuple(__k),
    ...
    pair<iterator, bool> try_emplace(key_type&& __k, _Args&&... __args) {
      ...
      if (__i == end() || key_comp()(__k, (*__i).first)) {
        __i =
            emplace_hint(__i, std::piecewise_construct,
                         std::forward_as_tuple(std::move(__k)),
                         std::forward_as_tuple(std::forward<_Args>(__args)...));
        return {__i, true};
      }
    ...
    iterator try_emplace(const_iterator __hint, const key_type& __k,
      ...
      if (__true_hint.second)
        __i =
            emplace_hint(iterator(__true_hint.second), std::piecewise_construct,
    ...
    template <typename _Pair>
    __enable_if_t<is_constructible<value_type, _Pair>::value, iterator> insert(
    ...
    iterator erase(iterator __position) { return _M_t.erase(__position); }
    // @}
#else
    /**
     *  @brief Erases an element from a %map.
    ...
    template <typename _Kt>
    auto contains(const _Kt& __x) const
...
#if __cplusplus > 201103L
    template <typename _Kt>
    auto lower_bound(const _Kt& __x) const
...
#if __cplusplus > 201103L
    template <typename _Kt>
    auto upper_bound(const _Kt& __x)
...
#if __cplusplus > 201103L
    template <typename _Kt>
    auto upper_bound(const _Kt& __x) const
...
#if __cplusplus > 201103L
    template <typename _Kt>
    auto equal_range(const _Kt& __x) const -> decltype(
        pair<const_iterator, const_iterator>(_M_t._M_equal_range_tr(__x))) {
    ...
    template <typename _K1, typename _T1, typename _C1, typename _A1>
    friend __detail::__synth3way_t<pair<const _K1, _T1>> operator<=>(
  ...
  template <typename _Key, typename _Val, typename _Cmp1, typename _Alloc,
            typename _Cmp2>
  struct _Rb_tree_merge_helper<_GLIBCXX_STD_C::map<_Key, _Val, _Cmp1, _Alloc>,
                               _Cmp2> {
   private:
    friend class _GLIBCXX_STD_C::map<_Key, _Val, _Cmp1, _Alloc>;

    static auto& _S_get_tree(
        _GLIBCXX_STD_C::map<_Key, _Val, _Cmp2, _Alloc>& __map) {
      return __map._M_t;
    }

    static auto& _S_get_tree(
        _GLIBCXX_STD_C::multimap<_Key, _Val, _Cmp2, _Alloc>& __map) {
      return __map._M_t;
    }
  };
#endif  // C++17
