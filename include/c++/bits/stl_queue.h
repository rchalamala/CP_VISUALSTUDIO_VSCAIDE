...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  class queue {
    ...
    __glibcxx_class_requires(_Tp, _SGIAssignableConcept)
        ...
        __glibcxx_class_requires(_Sequence, _FrontInsertionSequenceConcept)
            __glibcxx_class_requires(_Sequence, _BackInsertionSequenceConcept)
                __glibcxx_class_requires2(_Tp, _Sequence_value_type,
                                          _SameTypeConcept)
#endif
    ...
    friend compare_three_way_result_t<_Seq1> operator<=>(
    ...
    using _Uses =
        typename enable_if<uses_allocator<_Sequence, _Alloc>::value>::type;
   ...
   public:
    ...
    template <typename _Seq = _Sequence,
              typename _Requires = typename enable_if<
     ...
     */
    _GLIBCXX_NODISCARD bool empty() const { return c.empty(); }
  ...
  inline
      ...
      void
#endif
      swap(queue<_Tp, _Seq> & __x,
           queue<_Tp, _Seq> & __y) noexcept(noexcept(__x.swap(__y))) {
    __x.swap(__y);
  }
  ...
  class priority_queue {
...
#if __cplusplus >= 201103L
                        template <typename _Alloc>
                        using _Uses = typename enable_if<
                            uses_allocator<_Sequence, _Alloc>::value>::type;
   ...
   public:
    ...
    template <typename _Seq = _Sequence,
              typename _Requires = typename enable_if<
                  __and_<is_default_constructible<_Compare>,
                         is_default_constructible<_Seq>>::value>::type>
...
#if __cplusplus >= 201103L
    void swap(priority_queue& __pq) noexcept(
        __and_<
#if __cplusplus > 201402L || !defined(__STRICT_ANSI__)  // c++1z or gnu++11
