...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  template <typename _Tp>
  inline mask_array<_Tp>::mask_array(_Array<_Tp> __a, size_t __s,
                                     _Array<bool> __m)
      : _M_sz(__s), _M_mask(__m), _M_array(__a) {}

  template <typename _Tp>
  inline mask_array<_Tp>& mask_array<_Tp>::operator=(
...
#define _DEFINE_VALARRAY_OPERATOR(_Op, _Name)                                  \
  template <typename _Tp>                                                      \
  inline void mask_array<_Tp>::operator _Op##=(const valarray<_Tp>& __v)       \
      const {                                                                  \
    _Array_augmented_##_Name(_M_array, _M_mask, _Array<_Tp>(__v), __v.size()); \
  }                                                                            \
                                                                               \
  template <typename _Tp>                                                      \
  template <class _Dom>                                                        \
  inline void mask_array<_Tp>::operator _Op##=(const _Expr<_Dom, _Tp>& __e)    \
      const {                                                                  \
