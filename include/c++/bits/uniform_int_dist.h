...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  concept uniform_random_bit_generator =
      invocable<_Gen&>&& unsigned_integral<invoke_result_t<_Gen&>>&& requires {
    { _Gen::min() }
    ->same_as<invoke_result_t<_Gen&>>;
    { _Gen::max() }
    ->same_as<invoke_result_t<_Gen&>>;
  ...
  uniform_int_distribution<_IntType>::operator()(
    ...
    if (__urngrange > __urange) {
      ...
      do __ret = __uctype(__urng()) - __urngmin;
      while (__ret >= __past);
      __ret /= __scaling;
  ...
  void uniform_int_distribution<_IntType>::__generate_impl(
    ...
    __glibcxx_function_requires(
        _ForwardIteratorConcept<_ForwardIterator>) typedef
        typename _UniformRandomNumberGenerator::result_type _Gresult_type;
    typedef typename std::make_unsigned<result_type>::type __utype;
    typedef typename std::common_type<_Gresult_type, __utype>::type __uctype;
    ...
    if (__urngrange > __urange) {
      ...
      */
      ...
      while (__f != __t) *__f++ = __uctype(__urng()) - __urngmin + __param.a();
  }
