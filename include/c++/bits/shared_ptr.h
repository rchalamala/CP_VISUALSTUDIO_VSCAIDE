...
#pragma GCC diagnostic pop
   ...
   private:
    ...
    friend shared_ptr<_Yp> allocate_shared(const _Alloc& __a,
                                           _Args&&... __args);
  ...
  template <typename _Tp>
  _GLIBCXX_NODISCARD inline bool operator==(const shared_ptr<_Tp>& __a,
                                            nullptr_t) noexcept {
    return !__a;
  }
  ...
  class weak_ptr : public __weak_ptr<_Tp> {
   ...
   public:
    ...
    template <typename _Yp>
    _Assignable<const weak_ptr<_Yp>&> operator=(
