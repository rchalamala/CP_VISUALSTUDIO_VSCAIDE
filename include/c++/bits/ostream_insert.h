...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  inline void __ostream_write(basic_ostream<_CharT, _Traits> & __out,
    ...
    if (__put != __n) __out.setstate(__ios_base::badbit);
  }
    ...
    if (__cerb) {
      __try {
        ...
        if (__w > __n) {
          ...
          if (!__left) __ostream_fill(__out, __w - __n);
          if (__out.good()) __ostream_write(__out, __s, __n);
          if (__left && __out.good()) __ostream_fill(__out, __w - __n);
        } else
      ...
      __catch(...) { __out._M_setstate(__ios_base::badbit); }
    }
    return __out;
  }
