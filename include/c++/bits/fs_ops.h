...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  inline void copy(const path& __from, const path& __to) {
    copy(__from, __to, copy_options::none);
  }

  inline void copy(const path& __from, const path& __to, error_code& __ec) {
  ...
  inline bool copy_file(const path& __from, const path& __to) {
  ...
  inline bool exists(file_status __s) noexcept {
    return status_known(__s) && __s.type() != file_type::not_found;
  }

  inline bool exists(const path& __p) { return exists(status(__p)); }

  inline bool exists(const path& __p, error_code& __ec) noexcept {
  ...
  inline bool is_fifo(const path& __p) { return is_fifo(status(__p)); }
  ...
  inline bool is_other(const path& __p) { return is_other(status(__p)); }
  ...
  inline bool is_socket(const path& __p) { return is_socket(status(__p)); }
  ...
  inline void permissions(const path& __p, perms __prms,
                          error_code& __ec) noexcept {
