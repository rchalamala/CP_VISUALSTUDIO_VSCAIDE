...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  using __is_encoded_char = __is_one_of<remove_const_t<_CharT>, char,
  ...
  template <typename _Iter, typename _Iter_traits = std::iterator_traits<_Iter>>
  ...
  static __is_path_iter_src<_Iter> __is_path_src(_Iter, int);

  template <typename _CharT, typename _Traits, typename _Alloc>
  static __is_encoded_char<_CharT> __is_path_src(
      const basic_string<_CharT, _Traits, _Alloc>&, int);

  template <typename _CharT, typename _Traits>
  static __is_encoded_char<_CharT> __is_path_src(
      const basic_string_view<_CharT, _Traits>&, int);

  template <typename _Unknown>
  static std::false_type __is_path_src(const _Unknown&, ...);

  template <typename _Tp1, typename _Tp2>
  struct __constructible_from;

  template <typename _Iter>
  struct __constructible_from<_Iter, _Iter> : __is_path_iter_src<_Iter> {};
  ...
  using _Path =
      typename std::enable_if<__and_<__not_<is_same<remove_cv_t<_Tp1>, path>>,
                                     __not_<is_void<remove_pointer_t<_Tp1>>>,
                                     __constructible_from<_Tp1, _Tp2>>::value,
                              path>::type;

  template <typename _Source>
  _Source _S_range_begin(_Source __begin) {
    return __begin;
  }
  ...
  template <typename _Source>
  __null_terminated _S_range_end(_Source) {
    return {};
  }

  template <typename _CharT, typename _Traits, typename _Alloc>
  inline const _CharT* _S_range_begin(
      const basic_string<_CharT, _Traits, _Alloc>& __str) {
    return __str.data();
  }

  template <typename _CharT, typename _Traits, typename _Alloc>
  inline const _CharT* _S_range_end(
      const basic_string<_CharT, _Traits, _Alloc>& __str) {
  ...
  using __value_type_is_char =
      std::enable_if_t<std::is_same_v<_UnqualVal, char>, _UnqualVal>;
  ...
  using __value_type_is_char_or_char8_t =
      std::enable_if_t<__or_v<std::is_same<_UnqualVal, char>
#ifdef _GLIBCXX_USE_CHAR8_T
                              ,
  ...
  class path {
   public:
    ...
    __detail::_Path<_InputIterator, _InputIterator>& assign(
    ...
    template <typename _InputIterator>
    __detail::_Path<_InputIterator, _InputIterator>& append(
        _InputIterator __first, _InputIterator __last) {
    ...
    template <typename _InputIterator>
    __detail::_Path<_InputIterator, _InputIterator>& concat(
        _InputIterator __first, _InputIterator __last) {
...
#endif
#ifdef _GLIBCXX_USE_CHAR8_T
    __attribute__((__abi_tag__("__u8"))) std::u8string u8string() const;
#else
...
#endif
#ifdef _GLIBCXX_USE_CHAR8_T
    __attribute__((__abi_tag__("__u8"))) std::u8string generic_u8string() const;
#else
    ...
    friend std::basic_istream<_CharT, _Traits>& operator>>(
      ...
      if (__is >> std::quoted(__tmp)) __p = std::move(__tmp);
      return __is;
    }
    ...
    /// Compare paths
    friend strong_ordering operator<=>(const path& __lhs,
    ...
    static std::basic_string<_CharT> _S_string_from_iter(
        _InputIterator __source) {
   ...
   private:
    ...
    static basic_string_view<value_type> _S_convert(
        value_type* __src, __detail::__null_terminated) {
      return __src;
    }
    ...
    static basic_string_view<value_type> _S_convert(value_type* __first,
                                                    value_type* __last) {
      return {__first, __last - __first};
    }

    static basic_string_view<value_type> _S_convert(const value_type* __first,
                                                    const value_type* __last) {
      return {__first, __last - __first};
    }

    template <typename _Iter>
    static string_type _S_convert(_Iter __first, _Iter __last) {
      using __value_type = typename std::iterator_traits<_Iter>::value_type;
      return _Cvt<typename remove_cv<__value_type>::type>::_S_convert(__first,
                                                                      __last);
    }

    template <typename _InputIterator>
    static string_type _S_convert(_InputIterator __src,
    ...
    static string_type _S_convert_loc(const char* __first, const char* __last,
                                      const std::locale& __loc);

    template <typename _Iter>
    static string_type _S_convert_loc(_Iter __first, _Iter __last,
                                      const std::locale& __loc) {
    ...
    template <typename _InputIterator>
    static string_type _S_convert_loc(_InputIterator __src,
                                      __detail::__null_terminated,
                                      const std::locale& __loc) {
    ...
    static basic_string<_CharT, _Traits, _Allocator> _S_str_convert(
        basic_string_view<value_type>, const _Allocator&);
  ...
  inline path u8path(_InputIterator __first, _InputIterator __last) {
#ifdef _GLIBCXX_FILESYSTEM_IS_WINDOWS
   ...
   */
  template <
      typename _Source, typename _Require = __detail::_Path<_Source>,
      typename _CharT = __detail::__value_type_is_char_or_char8_t<_Source>>
  inline path u8path(const _Source& __source) {
#ifdef _GLIBCXX_FILESYSTEM_IS_WINDOWS
    if constexpr (is_same_v<_CharT, char>) {
  ...
  template <typename _CharT>
  struct path::_Cvt {
    static string_type _S_convert(const _CharT* __f, const _CharT* __l) {
#ifdef _GLIBCXX_FILESYSTEM_IS_WINDOWS
      std::wstring __wstr;
      if constexpr (is_same_v<_CharT, char>) {
        struct _UCvt : std::codecvt<wchar_t, char, std::mbstate_t> {
        } __cvt;
        if (__str_codecvt_in_all(__f, __l, __wstr, __cvt)) return __wstr;
      }
#ifdef _GLIBCXX_USE_CHAR8_T
        ...
        if (__str_codecvt_in_all(__f2, __l2, __wstr, __wcvt)) return __wstr;
      }
#endif
    ...
    static string_type _S_convert(_CharT* __f, _CharT* __l) {
    ...
    static string_type _S_convert(
        __gnu_cxx::__normal_iterator<_Iter, _Cont> __first,
  ...
  class path::iterator {
   ...
   private:
    ...
    friend difference_type __path_iter_distance(const iterator& __first,
    ...
    friend void __path_iter_advance(iterator& __i, difference_type __n) {
      if (__n == 1)
        ++__i;
      else if (__n == -1)
        --__i;
  ...
  inline path& path::operator=(path&& __p) noexcept {
  ...
  inline path& path::operator=(string_type&& __source) {
    return *this = path(std::move(__source));
  }

  inline path& path::assign(string_type&& __source) {
    return *this = path(std::move(__source));
  }

  inline path& path::operator+=(const string_type& __x) {
    _M_concat(__x);
    return *this;
  }

  inline path& path::operator+=(const value_type* __x) {
    _M_concat(__x);
    return *this;
  }

  inline path& path::operator+=(value_type __x) {
    _M_concat(basic_string_view<value_type>(&__x, 1));
    return *this;
  }

  inline path& path::operator+=(basic_string_view<value_type> __x) {
    _M_concat(__x);
    return *this;
  }

  template <typename _CharT>
  inline __detail::_Path<_CharT*, _CharT*>& path::operator+=(_CharT __x) {
  ...
  /// @cond undocumented
  template <typename _CharT, typename _Traits, typename _Allocator>
  std::basic_string<_CharT, _Traits, _Allocator> path::_S_str_convert(
      basic_string_view<value_type> __str, const _Allocator& __a) {
    static_assert(!is_same_v<_CharT, value_type>);

    using _WString = basic_string<_CharT, _Traits, _Allocator>;

    if (__str.size() == 0) return _WString(__a);
  ...
  template <typename _CharT, typename _Traits, typename _Allocator>
  inline basic_string<_CharT, _Traits, _Allocator> path::string(
...
#ifdef _GLIBCXX_USE_CHAR8_T
  inline std::u8string path::u8string() const { return string<char8_t>(); }
#else
  inline std::string path::u8string() const {
    ...
    if (__str_codecvt_out_all(__first, __last, __str, __cvt)) return __str;
    _GLIBCXX_THROW_OR_ABORT(
        filesystem_error("Cannot convert character sequence",
                         std::make_error_code(errc::illegal_byte_sequence)));
#else
...
#endif
    using _Alloc2 = typename allocator_traits<
        _Allocator>::template rebind_alloc<value_type>;
  ...
  inline int path::compare(const string_type& __s) const noexcept {
    return compare(basic_string_view<value_type>(__s));
  }

  inline int path::compare(const value_type* __s) const noexcept {
    return compare(basic_string_view<value_type>(__s));
  }
  ...
  inline path path::filename() const {
    if (empty())
      ...
      if (_M_pathname.back() == preferred_separator) return {};
      auto& __last = *--end();
      if (__last._M_type() == _Type::_Filename) return __last;
    }
    return {};
  }
  ...
  inline bool path::has_stem() const noexcept {
    auto ext = _M_find_extension();
    return ext.first && ext.second != 0;
  }

  inline bool path::has_extension() const noexcept {
    auto ext = _M_find_extension();
    return ext.first && ext.second != string_type::npos;
  }

  inline bool path::is_absolute() const noexcept {
#ifdef _GLIBCXX_FILESYSTEM_IS_WINDOWS
  ...
  inline path::iterator path::begin() const {
    if (_M_type() == _Type::_Multi) return iterator(this, _M_cmpts.begin());
    return iterator(this, empty());
  }

  inline path::iterator path::end() const {
    if (_M_type() == _Type::_Multi) return iterator(this, _M_cmpts.end());
  ...
  inline bool path::iterator::_M_equals(iterator __rhs) const {
    if (_M_path != __rhs._M_path) return false;
    if (_M_path == nullptr) return true;
  ...
  inline ptrdiff_t distance(filesystem::path::iterator __first,
                            filesystem::path::iterator __last) {
    return __path_iter_distance(__first, __last);
  }

  template <typename _InputIterator, typename _Distance>
  void advance(filesystem::path::iterator & __i, _Distance __n) {
