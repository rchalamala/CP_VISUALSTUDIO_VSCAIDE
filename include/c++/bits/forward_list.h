...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  struct _Fwd_list_node_base {
    ...
    _Fwd_list_node_base* _M_transfer_after(
        _Fwd_list_node_base* __begin, _Fwd_list_node_base* __end) noexcept {
  ...
  struct _Fwd_list_node : public _Fwd_list_node_base {
    ...
    const _Tp* _M_valptr() const noexcept { return _M_storage._M_ptr(); }
  };
  ...
  struct _Fwd_list_iterator {
    ...
    _Fwd_list_iterator() noexcept : _M_node() {}

    explicit _Fwd_list_iterator(_Fwd_list_node_base* __n) noexcept
        : _M_node(__n) {}
  ...
  struct _Fwd_list_const_iterator {
    ...
    _Fwd_list_const_iterator() noexcept : _M_node() {}

    explicit _Fwd_list_const_iterator(const _Fwd_list_node_base* __n) noexcept
        : _M_node(__n) {}
  ...
  struct _Fwd_list_base {
   ...
   public:
    ...
    _Node_alloc_type& _M_get_Node_allocator() noexcept { return this->_M_impl; }
    ...
    _Fwd_list_base(_Node_alloc_type&& __a) : _M_impl(std::move(__a)) {}
    ...
    ~_Fwd_list_base() { _M_erase_after(&_M_impl._M_head, nullptr); }
   ...
   protected:
    ...
    template <typename... _Args>
    _Fwd_list_node_base* _M_insert_after(const_iterator __pos,
                                         _Args&&... __args);
  ...
  class forward_list : private _Fwd_list_base<_Tp, _Alloc> {
    ...
    static_assert(
        is_same<typename _Alloc::value_type, _Tp>::value,
   ...
   public:
     ...
     */
    explicit forward_list(const _Alloc& __al) noexcept
   ...
   private:
    forward_list(forward_list&& __list, _Node_alloc_type&& __al, false_type)
   ...
   public:
     ...
     *  @param  __al    An allocator object.
     */
    forward_list(forward_list&& __list, const _Alloc& __al) noexcept(
     ...
     */
    explicit forward_list(size_type __n, const _Alloc& __al = _Alloc())
        : _Base(_Node_alloc_type(__al)) {
     ...
     */
    forward_list& operator=(forward_list&& __list) noexcept(
        _Node_alloc_traits::_S_nothrow_move()) {
    ...
    emplace_front(_Args&&... __args) {
      this->_M_insert_after(cbefore_begin(), std::forward<_Args>(__args)...);
#if __cplusplus > 201402L
      return front();
#endif
    }
    ...
    iterator emplace_after(const_iterator __pos, _Args&&... __args) {
      return iterator(
          this->_M_insert_after(__pos, std::forward<_Args>(__args)...));
    }
     ...
     */
    iterator insert_after(const_iterator __pos, size_type __n,
                          const _Tp& __val);
    ...
    template <typename _InputIterator,
              typename = std::_RequireInputIter<_InputIterator>>
    iterator insert_after(const_iterator __pos, _InputIterator __first,
...
#define _GLIBCXX_FWDLIST_REMOVE_RETURN_TYPE_TAG
   ...
   public:
     ...
     */
    _GLIBCXX_FWDLIST_REMOVE_RETURN_TYPE_TAG
    __remove_return_type unique() { return unique(std::equal_to<_Tp>()); }
...
#undef _GLIBCXX_FWDLIST_REMOVE_RETURN_TYPE_TAG
    ...
    void merge(forward_list& __list) { merge(std::move(__list)); }
     ...
     */
    void reverse() noexcept { this->_M_impl._M_head._M_reverse_after(); }
   ...
   private:
    ...
    iterator _M_splice_after(const_iterator __pos, const_iterator __before,
    ...
    // Called by resize(sz).
    void _M_default_insert_after(const_iterator __pos, size_type __n);

    // Called by operator=(forward_list&&)
    void _M_move_assign(forward_list&& __list, true_type) noexcept {
      clear();
    ...
    // Called by operator=(forward_list&&)
    void _M_move_assign(forward_list&& __list, false_type) {
    ...
    // CopyAssignable.
    template <typename _InputIterator>
    void _M_assign(_InputIterator __first, _InputIterator __last, true_type) {
    ...
    // CopyAssignable.
    template <typename _InputIterator>
    void _M_assign(_InputIterator __first, _InputIterator __last, false_type) {
      clear();
      insert_after(cbefore_begin(), __first, __last);
    }

    // Called by assign(size_type, const _Tp&) if Tp is CopyAssignable
    void _M_assign_n(size_type __n, const _Tp& __val, true_type) {
      auto __prev = before_begin();
      auto __curr = begin();
      auto __end = end();
    ...
    void _M_assign_n(size_type __n, const _Tp& __val, false_type) {
      clear();
   ...
   */
  template <typename _Tp, typename _Alloc>
  bool operator==(const forward_list<_Tp, _Alloc>& __lx,
   ...
   */
  template <typename _Tp, typename _Alloc>
  inline __detail::__synth3way_t<_Tp> operator<=>(
   ...
   */
  template <typename _Tp, typename _Alloc>
  inline bool operator<(const forward_list<_Tp, _Alloc>& __lx,
                        const forward_list<_Tp, _Alloc>& __ly) {
