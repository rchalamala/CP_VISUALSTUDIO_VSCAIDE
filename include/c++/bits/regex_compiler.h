...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  class _Compiler {
   public:
    ...
    shared_ptr<const _RegexT> _M_get_nfa() { return std::move(_M_nfa); }
   ...
   private:
    ...
    bool _M_match_token(_TokenT __token);
    ...
    bool _M_bracket_expression();

    template <bool __icase, bool __collate>
    void _M_insert_any_matcher_ecma();

    template <bool __icase, bool __collate>
    void _M_insert_any_matcher_posix();

    template <bool __icase, bool __collate>
    void _M_insert_char_matcher();

    template <bool __icase, bool __collate>
    void _M_insert_character_class_matcher();

    template <bool __icase, bool __collate>
    void _M_insert_bracket_matcher(bool __neg);

    // Returns true if successfully matched one term and should continue.
    // Returns false if the compiler should move on.
    template <bool __icase, bool __collate>
    bool _M_expression_term(
        pair<bool, _CharT>& __last_char,
        _BracketMatcher<_TraitsT, __icase, __collate>& __matcher);

    int _M_cur_int_value(int __radix);
  ...
  inline __enable_if_contiguous_iter<_FwdIter, _TraitsT> __compile_nfa(
  ...
  template <typename _TraitsT, typename _FwdIter>
  inline __disable_if_contiguous_iter<_FwdIter, _TraitsT> __compile_nfa(
      _FwdIter __first, _FwdIter __last,
      const typename _TraitsT::locale_type& __loc,
      regex_constants::syntax_option_type __flags) {
  ...
  class _RegexTranslatorBase {
   public:
    ...
    explicit _RegexTranslatorBase(const _TraitsT& __traits)
        : _M_traits(__traits) {}

    _CharT _M_translate(_CharT __ch) const {
      if (__icase)
    ...
    _StrTransT _M_transform(_CharT __ch) const {
    ...
    bool _M_match_range(const _StrTransT& __first, const _StrTransT& __last,
  ...
  class _RegexTranslator<_TraitsT, __icase, false>
   ...
   public:
    ...
    _StrTransT _M_transform(_CharT __ch) const { return __ch; }

    bool _M_match_range(_CharT __first, _CharT __last, _CharT __ch) const {
      if (!__icase) return __first <= __ch && __ch <= __last;
  ...
  class _RegexTranslator<_TraitsT, false, false> {
   public:
    ...
    explicit _RegexTranslator(const _TraitsT&) {}

    _CharT _M_translate(_CharT __ch) const { return __ch; }
  ...
  struct _AnyMatcher<_TraitsT, false, __icase, __collate> {
    ...
    explicit _AnyMatcher(const _TraitsT& __traits) : _M_translator(__traits) {}
  ...
  struct _AnyMatcher<_TraitsT, true, __icase, __collate> {
    ...
    bool _M_apply(_CharT __ch, true_type) const {
    ...
    bool _M_apply(_CharT __ch, false_type) const {
      auto __c = _M_translator._M_translate(__ch);
      auto __n = _M_translator._M_translate('\n');
      auto __r = _M_translator._M_translate('\r');
  ...
  struct _BracketMatcher {
   ...
   public:
    ...
    _StringT _M_add_collate_element(const _StringT& __s) {
      auto __st =
          _M_traits.lookup_collatename(__s.data(), __s.data() + __s.size());
      if (__st.empty())
    ...
    void _M_add_equivalence_class(const _StringT& __s) {
      ...
      __st =
          _M_traits.transform_primary(__st.data(), __st.data() + __st.size());
    ...
    void _M_add_character_class(const _StringT& __s, bool __neg) {
      auto __mask = _M_traits.lookup_classname(
          __s.data(), __s.data() + __s.size(), __icase);
      if (__mask == 0)
        __throw_regex_error(regex_constants::error_collate,
   ...
   private:
    ...
    static constexpr size_t _S_cache_size =
        1ul << (sizeof(_CharT) * __CHAR_BIT__ * int(_UseCache::value));

    struct _Dummy {};
    typedef
        typename std::conditional<_UseCache::value, std::bitset<_S_cache_size>,
                                  _Dummy>::type _CacheT;
    ...
    void _M_make_cache(true_type) {
      for (unsigned __i = 0; __i < _M_cache.size(); __i++)
        _M_cache[__i] = _M_apply(static_cast<_CharT>(__i), false_type());
    }

    void _M_make_cache(false_type) {}
