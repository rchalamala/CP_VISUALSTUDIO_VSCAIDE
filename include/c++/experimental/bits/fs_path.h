...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  /// @cond undocumented
  namespace __detail {
  template <typename _CharT, typename _Ch = typename remove_const<_CharT>::type>
  ...
  using _Path = typename std::enable_if<
      __and_<__not_<is_same<_Tp1_nocv, path> >, __not_<is_void<_Tp1_noptr> >,
             __constructible_from<_Tp1, _Tp2> >::value,
      path>::type;

  template <typename _Source>
  inline _Source _S_range_begin(_Source __begin) {
    return __begin;
  }
  ...
  template <typename _Source>
  inline __null_terminated _S_range_end(_Source) {
    return {};
  }
  ...
  using __value_type_is_char =
      typename std::enable_if<std::is_same<_UnqualVal, char>::value,
                              _UnqualVal>::type;
  ...
  using __value_type_is_char_or_char8_t =
      typename std::enable_if<__or_<std::is_same<_UnqualVal, char>
#ifdef _GLIBCXX_USE_CHAR8_T
                                    ,
  ...
  class path {
   public:
    ...
    path(path&& __p) noexcept
      ...
      if (_M_type == _Type::_Multi) _M_split_cmpts();
      __p.clear();
    }

    path(string_type&& __source) : _M_pathname(std::move(__source)) {
      _M_split_cmpts();
    }
   ...
   private:
    ...
    static string_type _S_convert(value_type* __src,
                                  __detail::__null_terminated) {
      return string_type(__src);
    }

    static string_type _S_convert(const value_type* __src,
                                  __detail::__null_terminated) {
      return string_type(__src);
    }
  ...
  basic_ostream<_CharT, _Traits>& operator<<(
      basic_ostream<_CharT, _Traits>& __os, const path& __p) {
  ...
  basic_istream<_CharT, _Traits>& operator>>(
      basic_istream<_CharT, _Traits>& __is, path& __p) {
...
#ifdef _GLIBCXX_FILESYSTEM_IS_WINDOWS
  template <typename _InputIterator>
  inline path __u8path(_InputIterator __first, _InputIterator __last, char) {
...
#ifdef _GLIBCXX_USE_CHAR8_T
  template <typename _InputIterator>
  inline path __u8path(_InputIterator __first, _InputIterator __last, char8_t) {
  ...
  /// Create a path from a UTF-8-encoded sequence of char
#ifdef _GLIBCXX_FILESYSTEM_IS_WINDOWS
  inline path __u8path(const string& __s, char) {
...
#ifdef _GLIBCXX_USE_CHAR8_T
  template <typename _Source>
  inline path __u8path(const _Source& __source, char8_t) {
  ...
  struct path::_Cvt {
#ifdef _GLIBCXX_FILESYSTEM_IS_WINDOWS
#ifdef _GLIBCXX_USE_CHAR8_T
    static string_type _S_wconvert(const char8_t* __f, const char8_t* __l,
                                   const char8_t*) {
    ...
    static string_type _S_wconvert(const char* __f, const char* __l,
                                   const char*) {
    ...
    static string_type _S_wconvert(const _CharT* __f, const _CharT* __l,
                                   const void*) {
  ...
  inline std::basic_string<_CharT, _Traits, _Allocator> path::string(
    ...
    if (__str_codecvt_out_all(__first, __last, __u8str, __cvt)) {
      struct {
        const _String* operator()(const _String& __from, _String&, true_type) {
...
#endif
    {
      ...
      _WString __wstr(__a);
      if (__str_codecvt_in_all(__first, __last, __wstr, __cvt)) return __wstr;
    }
#endif
    _GLIBCXX_THROW_OR_ABORT(
        filesystem_error("Cannot convert character sequence",
  ...
  inline int path::compare(const string_type& __s) const {
    return compare(path(__s));
  }

  inline int path::compare(const value_type* __s) const {
    return compare(path(__s));
  }

#if __cplusplus >= 201402L
  inline int path::compare(basic_string_view<value_type> __s) const {
    return compare(path(__s));
  }
#endif

  inline path path::filename() const { return empty() ? path() : *--end(); }
  ...
  inline bool path::iterator::_M_equals(iterator __rhs) const {
    ...
    if (_M_path->_M_type == path::_Type::_Multi) return _M_cur == __rhs._M_cur;
