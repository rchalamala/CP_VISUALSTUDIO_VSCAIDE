...
#include <atomic>
// This header defines the minimum set of parallel routines required to support
// Parallel STL, implemented on top of Intel(R) Threading Building Blocks
// (Intel(R) TBB) library
...
// parallel_find
//-----------------------------------------------------------------------
/** Return extremum value returned by brick f[i,j) for subranges [i,j) of
[first,last) Each f[i,j) must return a value in [i,j). */
...
_Index __parallel_find(_ExecutionPolicy&& __exec, _Index __first, _Index __last,
  ...
  __par_backend::__parallel_for(
      ...
      [__comp, __f, __first, &__extremum](_Index __i, _Index __j) {
        // See "Reducing Contention Through Priority Updates", PPoPP '13, for
        // discussion of why using a shared variable scales fairly well in this
        // situation.
        if (__comp(__i - __first, __extremum)) {
          _Index __res = __f(__i, __j);
          // If not '__last' returned then we found what we want so put this to
          // extremum
          if (__res != __j) {
...
// parallel_or
//------------------------------------------------------------------------
//! Return true if brick f[i,j) returns true for some subrange [i,j) of
//! [first,last)
