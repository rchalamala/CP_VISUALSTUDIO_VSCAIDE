...
//! Merge sequences [__xs,__xe) and [__ys,__ye) to output sequence
//! [__zs,(__xe-__xs)+(__ye-__ys)), using std::move
...
struct __serial_move_merge {
  ...
  explicit __serial_move_merge(std::size_t __nmerge, _MoveValues __move_values,
                               _MoveSequences __move_sequences)
      : _M_nmerge(__nmerge),
        _M_move_values(__move_values),
        _M_move_sequences(__move_sequences) {}
...
void __init_buf(_RandomAccessIterator1 __xs, _RandomAccessIterator1 __xe,
                _OutputIterator __zs, bool __bMove) {
  ...
  if (__bMove) {
    ...
    for (; __zs != __ze; ++__zs) new (&*__zs) _ValueType;
  }
}
...
class __stack {
  typedef typename std::iterator_traits<decltype(_Buf(0).get())>::value_type
      _ValueType;
  typedef typename std::iterator_traits<_ValueType*>::difference_type
      _DifferenceType;
  ...
  void operator=(const __stack&) = delete;

 public:
  __stack(_DifferenceType __max_size)
      : _M_buf(__max_size), _M_maxsize(__max_size) {
    _M_ptr = _M_buf.get();
  }
