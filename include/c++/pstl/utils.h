...
typename std::result_of<_Fp()>::type __except_handler(_Fp __f) {
  try {
    ...
    throw;  // re-throw bad_alloc according to the standard
...
template <typename _Fp>
void __invoke_if(std::false_type, _Fp __f) {}
...
template <typename _Fp>
void __invoke_if_not(std::true_type, _Fp __f) {}
...
//! "==" comparison.
/** Not called "equal" to avoid (possibly unfounded) concerns about accidental
   invocation via argument-dependent name lookup by code expecting to find the
   usual std::equal. */
class __pstl_equal {
 public:
