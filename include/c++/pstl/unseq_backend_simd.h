...
_Iterator __simd_walk_1(_Iterator __first, _DifferenceType __n,
  ...
  _PSTL_PRAGMA_SIMD
  for (_DifferenceType __i = 0; __i < __n; ++__i) __f(__first[__i]);
...
bool __simd_or(_Index __first, _DifferenceType __n, _Pred __pred) noexcept {
  ...
  while (__last != __first) {
    ...
    for (_DifferenceType __i = 0; __i < __block_size; ++__i)
      if (__pred(*(__first + __i))) __flag = 0;
    if (!__flag) return true;

    __first += __block_size;
    if (__last - __first >= __block_size << 1) {
      // Double the block _Size.  Any unnecessary iterations can be amortized
      // against work done so far.
      __block_size <<= 1;
    } else {
...
_Index __simd_first(_Index __first, _DifferenceType __begin,
  ...
  while (__end - __begin >= __block_size) {
    ...
    _PSTL_PRAGMA_SIMD_REDUCTION(|
                                : __found) for (_DifferenceType __i = __begin;
                                                __i < __begin + __block_size;
                                                ++__i) {
...
std::pair<_Index1, _Index2> __simd_first(_Index1 __first1, _DifferenceType __n,
  ...
  _PSTL_PRAGMA_SIMD_EARLYEXIT
  for (; __i < __n; ++__i)
    if (__pred(__first1[__i], __first2[__i])) break;
  ...
  while (__last1 - __first1 >= __block_size) {
    ...
    _PSTL_PRAGMA_VECTOR_UNALIGNED  // Do not generate peel loop part
    _PSTL_PRAGMA_SIMD_REDUCTION(|
                                : __found) for (__i = 0; __i < __block_size;
                                                ++__i) {
...
_Index __simd_fill_n(_Index __first, _DifferenceType __n,
  ...
  _PSTL_USE_NONTEMPORAL_STORES_IF_ALLOWED
  _PSTL_PRAGMA_SIMD
  for (_DifferenceType __i = 0; __i < __n; ++__i) __first[__i] = __value;
  return __first + __n;
}
...
_Index __simd_generate_n(_Index __first, _DifferenceType __size,
  ...
  _PSTL_USE_NONTEMPORAL_STORES_IF_ALLOWED
  _PSTL_PRAGMA_SIMD
  for (_DifferenceType __i = 0; __i < __size; ++__i) __first[__i] = __g();
...
_Index __simd_adjacent_find(_Index __first, _Index __last,
...
#if _PSTL_EARLYEXIT_PRESENT
  // Some compiler versions fail to compile the following loop when iterators
  // are used. Indices are used instead
  const _DifferenceType __n = __last - __first - 1;
  _PSTL_PRAGMA_VECTOR_UNALIGNED
  _PSTL_PRAGMA_SIMD_EARLYEXIT
  for (; __i < __n; ++__i)
    if (__pred(__first[__i], __first[__i + 1])) break;
  ...
  while (__last - __first >= __block_size) {
    ...
    _PSTL_PRAGMA_VECTOR_UNALIGNED  // Do not generate peel loop part
    _PSTL_PRAGMA_SIMD_REDUCTION(|
                                : __found) for (__i = 0; __i < __block_size - 1;
                                                ++__i) {
    ...
    if (__found) {
      if (__or_semantic) return __first;

      // This will vectorize
      for (__i = 0; __i < __block_size; ++__i)
        if (__lane[__i]) break;
      return __first + __i;  // As far as found is true a __result (__lane[__i]
                             // is true) is guaranteed
    }
    __first += __block_size;
  }
  // Process the rest elements
  for (; __last - __first > 1; ++__first)
    if (__pred(*__first, *(__first + 1))) return __first;
...
__simd_transform_reduce(_DifferenceType __n, _Tp __init, _BinaryOperation,
  ...
  for (_DifferenceType __i = 0; __i < __n; ++__i) __init += __f(__i);
  return __init;
}
...
// As soon as we cannot call __binary_op in "combiner" we create a wrapper over
// _Tp to encapsulate __binary_op
...
struct _Combiner {
  ...
  _Combiner(const _Tp& value, const _BinaryOp* bin_op)
      : __value(value), __bin_op(const_cast<_BinaryOp*>(bin_op)) {}
...
// [restriction] - std::iterator_traits<_ForwardIterator>::value_type should be
// DefaultConstructible. complexity [violation] - We will have at most (__n-1 +
// number_of_lanes) comparisons instead of at most __n-1.
...
// [restriction] - std::iterator_traits<_ForwardIterator>::value_type should be
// DefaultConstructible. complexity [violation] - We will have at most
// (2*(__n-1) + 4*number_of_lanes) comparisons instead of at most [1.5*(__n-1)].
template <typename _ForwardIterator, typename _Size, typename _Compare>
std::pair<_ForwardIterator, _ForwardIterator> __simd_minmax_element(
    _ForwardIterator __first, _Size __n, _Compare __comp) noexcept {
  if (__n == 0) {
  ...
  struct _ComplexType {
    ...
    _ComplexType(const _ValueType& min_val, const _ValueType& max_val,
                 const _Compare* comp)
        : __min_val(min_val),
          __max_val(max_val),
          __min_ind(0),
...
std::pair<_OutputIterator1, _OutputIterator2> __simd_partition_copy(
    _InputIterator __first, _DifferenceType __n, _OutputIterator1 __out_true,
...
template <class _ForwardIterator1, class _ForwardIterator2,
          class _BinaryPredicate>
_ForwardIterator1 __simd_find_first_of(_ForwardIterator1 __first,
  ...
  // Common case
  // If first sequence larger than second then we'll run simd_first with
  // parameters of first sequence. Otherwise, vice versa.
  if (__n1 < __n2) {
...
_RandomAccessIterator __simd_remove_if(_RandomAccessIterator __first,
  ...
  // if we have in sequence only one element that pred(__current[1]) != false we
