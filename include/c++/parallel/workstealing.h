...
_Op __for_each_template_random_access_workstealing(
  ...
  _ThreadIndex __num_threads = __gnu_parallel::max<_ThreadIndex>(
      1, __gnu_parallel::min<_DifferenceType>(__length, __get_max_threads()));
...
#pragma omp flush(__busy)
      ...
      while (__my_job._M_first <= __my_job._M_last) {
        ...
        _DifferenceType __current_job = __fetch_and_add<_DifferenceType>(
        ...
        for (_DifferenceType __job_counter = 0;
             __job_counter < __chunk_size && __current_job <= __my_job._M_last;
             ++__job_counter) {
          // Yes: process it!
...
#pragma omp flush(__busy)
      ...
      if (__supposed_load > 0) {
        ...
        _DifferenceType __stolen_first = __fetch_and_add<_DifferenceType>(
        ...
        __my_job._M_last = __gnu_parallel::min(__stolen_try, __supposed_last);
        __my_job._M_load = __my_job._M_last - __my_job._M_first + 1;
