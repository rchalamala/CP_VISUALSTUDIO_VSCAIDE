...
struct __find_if_selector : public __generic_find_selector {
   ...
   */
  template <typename _RAIter1, typename _RAIter2, typename _Pred>
  std::pair<_RAIter1, _RAIter2> _M_sequential_algorithm(_RAIter1 __begin1,
                                                        _RAIter1 __end1,
                                                        _RAIter2 __begin2,
                                                        _Pred __pred) {
    return std::make_pair(find_if(__begin1, __end1, __pred, sequential_tag()),
                          __begin2);
  }
};
...
struct __adjacent_find_selector : public __generic_find_selector {
  ...
  std::pair<_RAIter1, _RAIter2> _M_sequential_algorithm(_RAIter1 __begin1,
    ...
    // Passed end iterator is one short.
    _RAIter1 __spot =
        adjacent_find(__begin1, __end1 + 1, __pred, sequential_tag());
    if (__spot == (__end1 + 1)) __spot = __end1;
...
struct __find_first_of_selector : public __generic_find_selector {
  ...
  bool operator()(_RAIter1 __i1, _RAIter2 __i2, _Pred __pred) {
    for (_FIterator __pos_in_candidates = _M_begin;
      ...
      if (__pred(*__i1, *__pos_in_candidates)) return true;
    return false;
  }
  ...
  std::pair<_RAIter1, _RAIter2> _M_sequential_algorithm(_RAIter1 __begin1,
                                                        ...
                                                        _RAIter2 __begin2,
                                                        _Pred __pred) {
    return std::make_pair(find_first_of(__begin1, __end1, _M_begin, _M_end,
