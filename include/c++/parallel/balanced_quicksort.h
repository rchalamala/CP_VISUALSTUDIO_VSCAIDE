...
typename std::iterator_traits<_RAIter>::difference_type __qsb_divide(
  ...
  _RAIter __pivot_pos = __median_of_three_iterators(
      __begin, __begin + (__end - __begin) / 2, __end - 1, __comp);
  ...
  _DifferenceType __split_pos =
      __parallel_partition(__begin, __end - 1, __pred, __num_threads);
...
void __qsb_conquer(_QSBThreadLocal<_RAIter>** __tls, _RAIter __begin,
                   _RAIter __end, _Compare __comp, _ThreadIndex __iam,
  ...
  _ThreadIndex __num_threads_leftside = std::max<_ThreadIndex>(
      1, std::min<_ThreadIndex>(__num_threads - 1,
                                __split_pos * __num_threads / __n));
...
#pragma omp section
      {
        __qsb_conquer(__tls, __begin, __begin + __split_pos, __comp, __iam,
                      __num_threads_leftside, __wait);
        __wait = __parent_wait;
      }
...
void __qsb_local_sort_with_helping(_QSBThreadLocal<_RAIter>** __tls,
  ...
  _DifferenceType __base_case_n = _Settings::get().sort_qsb_base_case_maximal_n;
    ...
    if (__n > __base_case_n) {
      ...
      if (((__split_pos1 + 1) - __begin) < (__end - (__split_pos2))) {
        ...
        if (__begin != __split_pos1)
          __tl._M_leftover_parts.push_front(
              std::make_pair(__begin, __split_pos1));
      ...
      if (__tl._M_leftover_parts.pop_front(__current)) continue;
...
#pragma omp atomic
      *__tl._M_elements_leftover -= __elements_done;
      ...
      while (__wait && *__tl._M_elements_leftover > 0 &&
        ...
        if (!__successfully_stolen) __yield();
...
#pragma omp flush
      ...
      if (omp_get_wtime() >= (__search_start + 1.0)) {
        sleep(1);
        _GLIBCXX_PARALLEL_ASSERT(omp_get_wtime() < (__search_start + 1.0));
      }
#endif
...
void __parallel_sort_qsb(_RAIter __begin, _RAIter __end, _Compare __comp,
  ...
  _DifferenceType __queue_size =
      (__num_threads * (_ThreadIndex)(__rd_log2(__n) + 1));
  ...
  for (_ThreadIndex __i = 0; __i < __num_threads; ++__i) delete __tls[__i];
  delete[] __tls;
}
}  // namespace __gnu_parallel
