...
typename std::iterator_traits<_RAIter>::difference_type __parallel_partition(
  ...
  // shared
  _GLIBCXX_VOLATILE _DifferenceType __left = 0, __right = __n - 1, __dist = __n,
...
#pragma omp single
    {
      ...
      if (__s.partition_chunk_share > 0.0)
        __chunk_size = std::max<_DifferenceType>(
            ...
            (double)__n * __s.partition_chunk_share / (double)__num_threads);
      else
...
#pragma omp single
      ...
      // Private.
      _DifferenceType __thread_left, __thread_left_border, __thread_right,
      ...
      while (!__iam_finished) {
        if (__thread_left > __thread_left_border) {
          ...
          if (__former_dist < __chunk_size) {
            ...
            __thread_left = __fetch_and_add(&__left, __chunk_size);
            __thread_left_border = __thread_left + (__chunk_size - 1);
          }
        }
        ...
        if (__thread_right < __thread_right_border) {
          ...
          if (__former_dist < __chunk_size) {
            ...
            __thread_right = __fetch_and_add(&__right, -__chunk_size);
            __thread_right_border = __thread_right - (__chunk_size - 1);
          }
        }
        ...
        while (__thread_left < __thread_right) {
          ...
          std::iter_swap(__begin + __thread_left, __begin + __thread_right);
          ++__thread_left;
          --__thread_right;
        }
      }
...
#pragma omp barrier
      ...
      _DifferenceType __leftold = __left,
...
#pragma omp barrier
      ...
      if (__thread_left <= __thread_left_border &&
        ...
        std::swap_ranges(__begin + __thread_left_border - (__chunk_size - 1),
...
#pragma omp flush(__left, __right)
  ...
  while (__final_left < __final_right) {
    ...
    if (__final_left == __final_right) break;
...
void __parallel_nth_element(_RAIter __begin, _RAIter __nth, _RAIter __end,
  ...
  const _Settings& __s = _Settings::get();
  _DifferenceType __minimum_length = std::max<_DifferenceType>(
      2, std::max(__s.nth_element_minimal_n, __s.partition_minimal_n));
  ...
  while (static_cast<_SequenceIndex>(__end - __begin) >= __minimum_length) {
    ...
    if (__pivot_pos != (__end - 1)) std::iter_swap(__pivot_pos, __end - 1);
    __pivot_pos = __end - 1;
    ...
    __gnu_parallel::__binder2nd<_Compare, _ValueType, _ValueType, bool> __pred(
        __comp, *__pivot_pos);
    ...
    if (__split_pos1 != __pivot_pos) std::iter_swap(__split_pos1, __pivot_pos);
    ...
    if ((__split_pos1 + 1 - __begin) < (__n >> 7) ||
      ...
      __gnu_parallel::__unary_negate<
          __gnu_parallel::__binder1st<_Compare, _ValueType, _ValueType, bool>,
          _ValueType>
          __pred(__gnu_parallel::__binder1st<_Compare, _ValueType, _ValueType,
                                             bool>(__comp, *__pivot_pos));

      // Find other end of pivot-equal range.
      __split_pos2 =
          __gnu_sequential::partition(__split_pos1 + 1, __end, __pred);
    } else
      // Only skip the pivot.
 ...
 *  @param __comp Comparator. */
template <typename _RAIter, typename _Compare>
void __parallel_partial_sort(_RAIter __begin, _RAIter __middle, _RAIter __end,
                             _Compare __comp) {
