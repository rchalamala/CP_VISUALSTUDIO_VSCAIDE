...
struct __replace_selector : public __generic_for_each_selector<_It> {
  ...
  explicit __replace_selector(const _Tp& __new_val) : __new_val(__new_val) {}

  /** @brief Functor execution.
   *  @param __v Current value.
   *  @param __i iterator referencing object. */
  bool operator()(_Tp& __v, _It __i) {
    if (*__i == __v) *__i = __new_val;
    return true;
  }
};
...
struct __replace_if_selector : public __generic_for_each_selector<_It> {
  ...
  /** @brief Constructor.
   *  @param __new_val Value to replace with. */
  explicit __replace_if_selector(const _Tp& __new_val) : __new_val(__new_val) {}

  /** @brief Functor execution.
   *  @param __o Operator.
   *  @param __i iterator referencing object. */
  bool operator()(_Op& __o, _It __i) {
    if (__o(*__i)) *__i = __new_val;
    return true;
  }
};
...
struct __inner_product_selector : public __generic_for_each_selector<_It> {
  ...
  explicit __inner_product_selector(_It __b1, _It2 __b2)
...
struct _DummyReduct {
  bool operator()(bool, bool) const { return true; }
};
...
struct __min_element_reduct {
  ...
  explicit __min_element_reduct(_Compare& __c) : __comp(__c) {}

  _It operator()(_It __x, _It __y) { return (__comp(*__x, *__y)) ? __x : __y; }
};
...
struct __max_element_reduct {
  ...
  explicit __max_element_reduct(_Compare& __c) : __comp(__c) {}

  _It operator()(_It __x, _It __y) { return (__comp(*__x, *__y)) ? __y : __x; }
};
...
struct __accumulate_binop_reduct {
  ...
  explicit __accumulate_binop_reduct(_BinOp& __b) : __binop(__b) {}
