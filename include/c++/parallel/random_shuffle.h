...
struct _DRandomShufflingGlobalData {
  ...
  /** @brief Constructor. */
  _DRandomShufflingGlobalData(_RAIter& __source) : _M_source(__source) {}
};
...
inline int __random_number_pow2(int __logp, _RandomNumberGenerator& __rng) {
...
template <typename _RAIter, typename _RandomNumberGenerator>
void __parallel_random_shuffle_drs_pu(
  ...
  for (_BinIndex __b = 0; __b < __sd->_M_num_bins + 1; ++__b) __dist[__b] = 0;
...
#pragma omp single
  {
    ...
    for (_BinIndex __s = 0; __s < __sd->_M_num_bins; ++__s)
      __gnu_sequential::partial_sum(
          __sd->_M_dist[__s + 1],
          __sd->_M_dist[__s + 1] + __d->_M_num_threads + 1,
          __sd->_M_dist[__s + 1]);
  }
...
#pragma omp barrier
  ...
  __sd->_M_temporaries[__iam] =
      static_cast<_ValueType*>(::operator new(sizeof(_ValueType) * __offset));
...
#pragma omp barrier
  ...
  for (_BinIndex __b = __d->_M_bins_begin; __b < __d->__bins_end; ++__b) {
    _ValueType *__begin = (__sd->_M_temporaries[__iam] +
    ...
    std::copy(__begin, __end,
              __sd->_M_source + __global_offset +
                  (__b == __d->_M_bins_begin
                       ? 0
                       : __sd->_M_dist[__b][__d->_M_num_threads]));
  }
...
void __parallel_random_shuffle_drs(
  ...
  if (__num_bins < __num_bins_cache) {
    ...
    // 2 TLB entries needed per bin.
    __num_bins =
        std::min(static_cast<_DifferenceType>(__s.TLB_size / 2), __num_bins);
#endif
    __num_bins = __round_up_to_pow2(__num_bins);
#if _GLIBCXX_RANDOM_SHUFFLE_CONSIDER_L1
  }
  ...
  __num_bins =
      __round_up_to_pow2(std::max<_BinIndex>(__num_threads, __num_bins));
...
#pragma omp single
    {
      ...
      for (_ThreadIndex __i = 0; __i < __num_threads; ++__i) {
        ...
        __bin_cursor +=
            (__i < __bin_split ? (__bin_chunk_length + 1) : __bin_chunk_length);
        __pus[__i].__bins_end = __bin_cursor;
        for (; __j < __bin_cursor; ++__j) __sd._M_bin_proc[__j] = __i;
  ...
  for (int __s = 0; __s < (__num_bins + 1); ++__s) delete[] __sd._M_dist[__s];
  delete[] __sd._M_dist;
 ...
 */
template <typename _RAIter, typename _RandomNumberGenerator>
void __sequential_random_shuffle(_RAIter __begin, _RAIter __end,
  ...
  if (__num_bins > 1) {
    ...
    for (int __b = 0; __b < __num_bins + 1; ++__b) __dist0[__b] = 0;
    ...
    // Sum up bins.
    __gnu_sequential::partial_sum(__dist0, __dist0 + __num_bins + 1, __dist0);

    for (int __b = 0; __b < __num_bins + 1; ++__b) __dist1[__b] = __dist0[__b];
    ...
    for (_DifferenceType __i = 0; __i < __n; ++__i) __target[__i].~_ValueType();
...
inline void __parallel_random_shuffle(
  ...
  typedef typename _TraitsType::difference_type _DifferenceType;
  _DifferenceType __n = __end - __begin;
  __parallel_random_shuffle_drs(__begin, __end, __n, __get_max_threads(),
                                __rng);
}
}  // namespace __gnu_parallel
