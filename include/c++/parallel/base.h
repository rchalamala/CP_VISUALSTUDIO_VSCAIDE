...
inline _ThreadIndex __get_max_threads() {
  _ThreadIndex __i = omp_get_max_threads();
  return __i > 1 ? __i : 1;
}

inline bool __is_parallel(const _Parallelism __p) { return __p != sequential; }
...
inline _Size __rd_log2(_Size __n) {
  _Size __k;
 ...
 *  @see __decode2
 */
inline _CASable __encode2(int __a,
                          int __b)  // must all be non-negative, actually
{
 ...
 *  @see __encode2
 */
inline void __decode2(_CASable __x, int& __a, int& __b) {
...
class __unary_negate : public std::unary_function<argument_type, bool> {
 protected:
  _Predicate _M_pred;

 public:
  explicit __unary_negate(const _Predicate& __x) : _M_pred(__x) {}

  bool operator()(const argument_type& __x) { return !_M_pred(__x); }
};
...
template <typename _Tp>
struct _Less<_Tp, _Tp> : public std::less<_Tp> {};
...
template <typename _Tp>
struct _Plus<_Tp, _Tp, _Tp> : public std::plus<_Tp> {};
...
template <typename _Tp>
struct _Multiplies<_Tp, _Tp, _Tp> : public std::multiplies<_Tp> {};
...
class _PseudoSequenceIterator {
 public:
  ...
  const _Tp& operator*() const { return _M_val; }

  const _Tp& operator[](_DifferenceType) const { return _M_val; }
