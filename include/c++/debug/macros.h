...
#define _GLIBCXX_DEBUG_VERIFY_COND_AT(_Cond, _ErrMsg, _File, _Line, _Func) \
  if (__builtin_is_constant_evaluated())                                   \
    ...
    break;                                                                 \
  if (!(_Cond))                                                            \
  __gnu_debug::_Error_formatter::_S_at(_File, _Line, _Func)._ErrMsg._M_error()
#else
#define _GLIBCXX_DEBUG_VERIFY_COND_AT(_Cond, _ErrMsg, _File, _Line, _Func) \
  if (!(_Cond))                                                            \
  __gnu_debug::_Error_formatter::_S_at(_File, _Line, _Func)._ErrMsg._M_error()
...
#define __glibcxx_check_valid_constructor_range(_First, _Last)        \
  __gnu_debug::__check_valid_range(_First, _Last, __FILE__, __LINE__, \
                                   __PRETTY_FUNCTION__)
...
#define __glibcxx_check_can_increment_range(_First1, _Last1, _First2)   \
  do {                                                                  \
    typename __gnu_debug::_Distance_traits<__decltype(_First1)>::__type \
        __dist;                                                         \
...
#define __glibcxx_check_insert_range(_Position, _First, _Last, _Dist) \
  ...
  _GLIBCXX_DEBUG_VERIFY(                                              \
      __gnu_debug::__foreign_iterator(_Position, _First, _Last),      \
...
#define __glibcxx_check_nonempty() \
  _GLIBCXX_DEBUG_VERIFY(           \
      !this->empty(),              \
      _M_message(__gnu_debug::__msg_empty)._M_sequence(*this, "this"))
...
#define __glibcxx_check_irreflexive2(_First, _Last)             \
  _GLIBCXX_DEBUG_VERIFY(                                        \
      _First == _Last || __gnu_debug::__is_irreflexive(_First), \
      _M_message(__gnu_debug::__msg_irreflexive_ordering)       \
          ._M_iterator_value_type(_First, "< operator type"))
#else
#define __glibcxx_check_irreflexive2(_First, _Last)
...
#define __glibcxx_check_irreflexive_pred2(_First, _Last, _Pred)             \
  _GLIBCXX_DEBUG_VERIFY(                                                    \
      _First == _Last || __gnu_debug::__is_irreflexive_pred(_First, _Pred), \
      _M_message(__gnu_debug::__msg_irreflexive_ordering)                   \
          ._M_instance(_Pred, "functor")                                    \
          ._M_iterator_value_type(_First, "ordered type"))
#else
...
#define __glibcxx_check_sorted_set_pred(_First1, _Last1, _First2, _Pred)  \
  __glibcxx_check_valid_range(_First1, _Last1);                           \
  _GLIBCXX_DEBUG_VERIFY(__gnu_debug::__check_sorted_set(                  \
                            __gnu_debug::__base(_First1),                 \
                            __gnu_debug::__base(_Last1), _First2, _Pred), \
                        _M_message(__gnu_debug::__msg_unsorted_pred)      \
                            ._M_iterator(_First1, #_First1)               \
                            ._M_iterator(_Last1, #_Last1)                 \
                            ._M_string(#_Pred))
...
#define __glibcxx_check_partitioned_lower(_First, _Last, _Value)            \
  __glibcxx_check_valid_range(_First, _Last);                               \
  _GLIBCXX_DEBUG_VERIFY(                                                    \
      __gnu_debug::__check_partitioned_lower(                               \
          __gnu_debug::__base(_First), __gnu_debug::__base(_Last), _Value), \
...
#define __glibcxx_check_heap(_First, _Last)                                    \
  _GLIBCXX_DEBUG_VERIFY(                                                       \
      std::__is_heap(__gnu_debug::__base(_First), __gnu_debug::__base(_Last)), \
...
#define __glibcxx_check_equal_allocs(_This, _Other)    \
  _GLIBCXX_DEBUG_VERIFY(                               \
      _This.get_allocator() == _Other.get_allocator(), \
      _M_message(__gnu_debug::__msg_equal_allocs)._M_sequence(_This, "this"))
