...
#define PB_DS_HASH_BASE                                           \
  detail::container_base_dispatch<                                \
      Key, Mapped, _Alloc, Tag,                                   \
      typename __gnu_cxx::typelist::append<                       \
          typename __gnu_cxx::typelist::create4<                  \
              Hash_Fn, Eq_Fn, Resize_Policy,                      \
              detail::integral_constant<int, Store_Hash> >::type, \
          Policy_Tl>::type>::type
...
class basic_hash_table : public PB_DS_HASH_BASE {
 ...
 protected:
  ...
  basic_hash_table(T0 t0, T1 t1, T2 t2, T3 t3) : base_type(t0, t1, t2, t3) {}
...
#define PB_DS_CC_HASH_BASE                                                 \
  basic_hash_table<                                                        \
      Key, Mapped, Hash_Fn, Eq_Fn, Resize_Policy, Store_Hash, cc_hash_tag, \
...
class cc_hash_table : public PB_DS_CC_HASH_BASE {
 ...
 public:
  ...
  template <typename It>
  cc_hash_table(It first, It last, const hash_fn& h) : base_type(h) {
  ...
  void swap(cc_hash_table& other) { base_type::swap(other); }
};
...
class gp_hash_table : public PB_DS_GP_HASH_BASE {
 ...
 public:
  ...
  /// Constructor taking some policy objects. r_hash_fn will be
  /// copied by the hash_fn object of the container object.
  gp_hash_table(const hash_fn& h) : base_type(h) {}

  /// Constructor taking some policy objects. r_hash_fn will be
  /// copied by the hash_fn object of the container object, and
  /// r_eq_fn will be copied by the eq_fn object of the container
  /// object.
  gp_hash_table(const hash_fn& h, const eq_fn& e) : base_type(h, e) {}
  ...
  /// will be copied by the hash_fn object of the container object.
  template <typename It>
  gp_hash_table(It first, It last, const hash_fn& h) : base_type(h) {
    base_type::copy_from_range(first, last);
  }
  ...
  void swap(gp_hash_table& other) { base_type::swap(other); }
};
//@} hash-based
...
class basic_branch : public PB_DS_BRANCH_BASE {
 ...
 protected:
  ...
  template <typename T0, typename T1, typename T2, typename T3>
  basic_branch(T0 t0, T1 t1, T2 t2, T3 t3) : base_type(t0, t1, t2, t3) {}
...
#define PB_DS_TREE_BASE                                             \
  basic_branch<Key, Mapped, Tag,                                    \
               ...
               typename __gnu_cxx::typelist::create2<               \
                   Cmp_Fn, PB_DS_TREE_NODE_AND_IT_TRAITS>::type,    \
               _Alloc>
...
class tree : public PB_DS_TREE_BASE {
 ...
 public:
  ...
  template <typename It>
  tree(It first, It last, const cmp_fn& c) : base_type(c) {
    base_type::copy_from_range(first, last);
  }
  ...
  void swap(tree& other) { base_type::swap(other); }
};
...
#define PB_DS_TRIE_BASE                                             \
  basic_branch<Key, Mapped, Tag,                                    \
               ...
               typename __gnu_cxx::typelist::create2<               \
                   _ATraits, PB_DS_TRIE_NODE_AND_IT_TRAITS>::type,  \
               _Alloc>
...
class trie : public PB_DS_TRIE_BASE {
 ...
 public:
  ...
  trie(const access_traits& t) : base_type(t) {}
  ...
  /// last_it will be inserted into the container object.
  template <typename It>
  trie(It first, It last, const access_traits& t) : base_type(t) {
    base_type::copy_from_range(first, last);
  }

  trie(const trie& other) : base_type((const base_type&)other) {}
  ...
  void swap(trie& other) { base_type::swap(other); }
};
//@} branch-based
...
class list_update : public PB_DS_LU_BASE {
 ...
 public:
  ...
  list_update(const list_update& other) : base_type((const base_type&)other) {}
  ...
  void swap(list_update& other) { base_type::swap(other); }
};
//@} list-based
#undef PB_DS_LU_BASE
