...
class trie_policy_base : public branch_policy<Node_CItr, Node_Itr, _Alloc> {
 ...
 protected:
  static size_type common_prefix_len(node_iterator, e_const_iterator,
  ...
  static bool less(e_const_iterator, e_const_iterator, e_const_iterator,
                   e_const_iterator, const access_traits&);
};
...
typename PB_DS_CLASS_C_DEC::size_type PB_DS_CLASS_C_DEC::common_prefix_len(
    node_iterator nd_it, e_const_iterator b_r, e_const_iterator e_r,
  ...
  size_type ret = 0;
  while (b_l != e_l) {
    if (r_traits.e_pos(*b_l) != r_traits.e_pos(*b_r)) return ret;
...
PB_DS_CLASS_T_DEC
typename PB_DS_CLASS_C_DEC::iterator PB_DS_CLASS_C_DEC::leftmost_it(
    node_iterator nd_it) {
  if (nd_it.num_children() == 0) return *nd_it;

  return leftmost_it(nd_it.get_child(0));
}

PB_DS_CLASS_T_DEC
typename PB_DS_CLASS_C_DEC::iterator PB_DS_CLASS_C_DEC::rightmost_it(
    node_iterator nd_it) {
...
PB_DS_CLASS_T_DEC
bool PB_DS_CLASS_C_DEC::less(e_const_iterator b_l, e_const_iterator e_l,
