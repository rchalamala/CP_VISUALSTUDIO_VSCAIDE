...
inline typename PB_DS_CLASS_C_DEC::iterator PB_DS_CLASS_C_DEC::find_by_order(
  ...
  while (true) {
    if (order > nd_it.get_metadata()) return ++base_type::rightmost_it(nd_it);

    const size_type num_children = nd_it.num_children();
    if (num_children == 0) return *nd_it;
...
PB_DS_CLASS_C_DEC::find_by_order(size_type order) const {
  return const_cast<PB_DS_CLASS_C_DEC*>(this)->find_by_order(order);
}

PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::size_type PB_DS_CLASS_C_DEC::order_of_key(
...
PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::size_type PB_DS_CLASS_C_DEC::order_of_prefix(
  ...
  while (true) {
    ...
    if (num_children == 0) {
      ...
      typename access_traits::const_iterator key_b = r_traits.begin(r_key);

      typename access_traits::const_iterator key_e = r_traits.end(r_key);

      return (base_type::less(key_b, key_e, b, e, r_traits)) ? ord + 1 : ord;
    }
    ...
    do {
      ...
      else if (!base_type::less(b, e, child_nd_it.valid_prefix().first,
                                child_nd_it.valid_prefix().second, r_traits))
        next_nd_it = child_nd_it;
    } while (i-- > 0);

    if (next_nd_it == end_nd_it) return ord;
...
PB_DS_CLASS_T_DEC
inline void PB_DS_CLASS_C_DEC::operator()(
    node_iterator nd_it, node_const_iterator /*end_nd_it*/) const {
  const size_type num_children = nd_it.num_children();
