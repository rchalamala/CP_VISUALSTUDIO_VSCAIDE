...
inline typename PB_DS_CLASS_C_DEC::point_iterator PB_DS_CLASS_C_DEC::push(
  ...
  if (base_type::m_p_max == 0 ||
      Cmp_Fn::operator()(base_type::m_p_max->m_value, r_val))
    base_type::m_p_max = p_nd;

  p_nd->m_p_next_sibling = base_type::m_p_root;

  if (base_type::m_p_root != 0) base_type::m_p_root->m_p_prev_or_parent = p_nd;
...
PB_DS_CLASS_C_DEC::link_with_next_sibling(node_pointer p_nd) {
  ...
  if (Cmp_Fn::operator()(p_nd->m_value, p_next->m_value)) {
    ...
    if (base_type::m_p_max == p_nd) base_type::m_p_max = p_next;
  ...
  if (base_type::m_p_max == p_next) base_type::m_p_max = p_nd;
...
PB_DS_CLASS_T_DEC
void PB_DS_CLASS_C_DEC::make_0_exposed() {
  ...
  if (p_res->m_p_next_sibling != 0 &&
      p_res->m_metadata == p_res->m_p_next_sibling->m_metadata)
    m_rc.push(p_res);
}
#endif
