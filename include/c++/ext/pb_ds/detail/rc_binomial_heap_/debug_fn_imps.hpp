...
void PB_DS_CLASS_C_DEC::assert_valid(const char* __file, int __line) const {
  ...
  while (p_nd != 0) {
    ...
    PB_DS_DEBUG_VERIFY(p_next->m_p_next_sibling == 0 ||
                       p_next->m_metadata <
                           p_next->m_p_next_sibling->m_metadata);
...
PB_DS_CLASS_C_DEC::next_2_pointer(node_const_pointer p_nd) {
  if (p_nd == 0) return 0;
  ...
  if (p_nd->m_metadata == p_next->m_metadata) return p_nd;
...
PB_DS_CLASS_T_DEC
typename PB_DS_CLASS_C_DEC::node_const_pointer
PB_DS_CLASS_C_DEC::next_after_0_pointer(node_const_pointer p_nd) {
  if (p_nd == 0) return 0;
  ...
  if (p_nd->m_metadata < p_next->m_metadata) return p_next;
