...
inline typename PB_DS_CLASS_C_DEC::point_iterator PB_DS_CLASS_C_DEC::find(
  ...
  if (p_found != base_type::m_p_head) splay(p_found);
...
inline typename PB_DS_CLASS_C_DEC::node_pointer PB_DS_CLASS_C_DEC::find_imp(
    key_const_reference r_key) {
  _GLIBCXX_DEBUG_ONLY(
      base_type::structure_only_assert_valid(__FILE__, __LINE__);)
  node_pointer p_nd = base_type::m_p_head->m_p_parent;
  while (p_nd != 0)
    if (!Cmp_Fn::operator()(PB_DS_V2F(p_nd->m_value), r_key)) {
      if (!Cmp_Fn::operator()(r_key, PB_DS_V2F(p_nd->m_value))) return p_nd;
      p_nd = p_nd->m_p_left;
    } else
      p_nd = p_nd->m_p_right;
...
PB_DS_CLASS_C_DEC::find_imp(key_const_reference r_key) const {
