...
PB_DS_CLASS_T_DEC
typename PB_DS_CLASS_C_DEC::node_allocator PB_DS_CLASS_C_DEC::s_node_allocator;
...
PB_DS_CLASS_T_DEC
PB_DS_CLASS_C_DEC::PB_DS_BIN_TREE_NAME(const Cmp_Fn& r_cmp_fn)
...
PB_DS_CLASS_T_DEC
PB_DS_CLASS_C_DEC::PB_DS_BIN_TREE_NAME(const PB_DS_CLASS_C_DEC& other)
    :
  ...
  __try {
    ...
    if (m_p_head->m_p_parent != 0) m_p_head->m_p_parent->m_p_parent = m_p_head;
    m_size = other.m_size;
    initialize_min_max();
  }
  __catch(...) {
...
PB_DS_CLASS_T_DEC
void PB_DS_CLASS_C_DEC::value_swap(PB_DS_CLASS_C_DEC& other) {
...
PB_DS_CLASS_T_DEC
void PB_DS_CLASS_C_DEC::initialize() {
...
PB_DS_CLASS_T_DEC
typename PB_DS_CLASS_C_DEC::node_pointer PB_DS_CLASS_C_DEC::recursive_copy_node(
  ...
  if (p_ret->m_p_left != 0) p_ret->m_p_left->m_p_parent = p_ret;

  if (p_ret->m_p_right != 0) p_ret->m_p_right->m_p_parent = p_ret;
...
PB_DS_CLASS_T_DEC
void PB_DS_CLASS_C_DEC::initialize_min_max() {
...

  {
    ...
    while (p_min->m_p_left != 0) p_min = p_min->m_p_left;
    m_p_head->m_p_left = p_min;
  }

  {
    node_pointer p_max = m_p_head->m_p_parent;
    while (p_max->m_p_right != 0) p_max = p_max->m_p_right;
