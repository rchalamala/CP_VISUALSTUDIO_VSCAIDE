...
PB_DS_CLASS_T_DEC
inline void PB_DS_CLASS_C_DEC::actual_erase_node(node_pointer p_nd) {
  _GLIBCXX_DEBUG_ASSERT(m_size > 0);
  --m_size;
  p_nd->~node();
...
PB_DS_CLASS_T_DEC
void PB_DS_CLASS_C_DEC::to_linked_list() {
...
typename PB_DS_CLASS_C_DEC::node_pointer PB_DS_CLASS_C_DEC::prune(Pred pred) {
  node_pointer p_cur = m_p_root;
  m_p_root = 0;
  ...
  while (p_cur != 0) {
    ...
    if (pred(p_cur->m_value)) {
      ...
      if (p_out != 0) p_out->m_p_prev_or_parent = p_cur;
      p_out = p_cur;
    } else {
      p_cur->m_p_next_sibling = m_p_root;
      if (m_p_root != 0) m_p_root->m_p_prev_or_parent = p_cur;
      m_p_root = p_cur;
    }
    p_cur = p_next;
  }
...
PB_DS_CLASS_T_DEC
void PB_DS_CLASS_C_DEC::bubble_to_top(node_pointer p_nd) {
