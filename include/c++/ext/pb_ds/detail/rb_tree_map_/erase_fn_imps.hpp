...
PB_DS_CLASS_T_DEC
void PB_DS_CLASS_C_DEC::remove_node(node_pointer p_z) {
  ...
  if (p_y->m_p_left == 0)
    ...
    p_y = p_y->m_p_right;
    while (p_y->m_p_left != 0) p_y = p_y->m_p_left;
    p_x = p_y->m_p_right;
  }

  if (p_y == p_z) {
    p_new_x_parent = p_y->m_p_parent;
    if (p_x != 0) p_x->m_p_parent = p_y->m_p_parent;
...
PB_DS_CLASS_T_DEC
void PB_DS_CLASS_C_DEC::remove_fixup(node_pointer p_x,
  ...
  while (p_x != base_type::m_p_head->m_p_parent && is_effectively_black(p_x))
    if (p_x == p_new_x_parent->m_p_left) {
      ...
      if (is_effectively_black(p_w->m_p_left) &&
        ...
        if (p_w->m_p_right != 0) p_w->m_p_right->m_red = false;
