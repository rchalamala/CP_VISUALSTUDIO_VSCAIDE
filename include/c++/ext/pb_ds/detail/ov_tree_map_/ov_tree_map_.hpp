...
#define PB_DS_TREE_TRACE_BASE_C_DEC                                          \
  tree_trace_base<typename Node_And_It_Traits::node_const_iterator,          \
                  typename Node_And_It_Traits::node_iterator, Cmp_Fn, false, \
                  _Alloc>
#endif
...
class PB_DS_OV_TREE_NAME :
 ...
 private:
  ...
  typedef
      typename metadata_alloc_traits::const_reference metadata_const_reference;
 ...
 public:
  ...
  class cond_dtor {
   public:
    ...
    ~cond_dtor() {
      ...
      if (m_max_size > 0) value_allocator().deallocate(m_a_vec, m_max_size);
    }

    inline void set_no_action() { m_no_action = true; }
  ...
  inline point_iterator lower_bound(key_const_reference r_key) {
  ...
  inline point_const_iterator lower_bound(key_const_reference r_key) const {
    return const_cast<PB_DS_CLASS_C_DEC&>(*this).lower_bound(r_key);
  }

  inline point_iterator upper_bound(key_const_reference r_key) {
  ...
  inline point_const_iterator upper_bound(key_const_reference r_key) const {
    return const_cast<PB_DS_CLASS_C_DEC&>(*this).upper_bound(r_key);
  }

  inline point_iterator find(key_const_reference r_key) {
    PB_DS_ASSERT_VALID((*this))
    iterator pot_it = lower_bound(r_key);
    if (pot_it != end() && !Cmp_Fn::operator()(r_key, PB_DS_V2F(*pot_it))) {
      PB_DS_CHECK_KEY_EXISTS(r_key)
  ...
  inline point_const_iterator find(key_const_reference r_key) const {
  ...
  inline iterator erase(iterator it) { return erase_imp<iterator>(it); }
  ...
  inline iterator begin() { return m_a_values; }

  inline const_iterator begin() const { return m_a_values; }

  inline iterator end() { return m_end_it; }

  inline const_iterator end() const { return m_end_it; }
 ...
 private:
  ...
  inline iterator insert_new_val(iterator it, const_reference r_value) {
