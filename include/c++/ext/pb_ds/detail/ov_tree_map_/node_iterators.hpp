...
class ov_tree_node_const_it_ {
 protected:
  ...
  typedef
      typename rebind_traits<_Alloc, Value_Type>::const_pointer const_pointer;
 ...
 protected:
  ...
  inline static Ptr mid_pointer(Ptr p_begin, Ptr p_end) {
 ...
 public:
  ...
  typedef typename rebind_traits<_Alloc, Value_Type>::const_pointer value_type;

  typedef typename rebind_traits<
      _Alloc, typename remove_const<Value_Type>::type>::const_pointer reference;

  typedef
      typename rebind_traits<_Alloc, typename remove_const<Value_Type>::type>::
 ...
 public:
  ...
  inline metadata_const_reference get_metadata() const {
    enum { has_metadata = !is_same<Metadata_Type, null_type>::value };
  ...
  inline this_type get_l_child() const {
    ...
    return (this_type(mid_pointer(m_p_begin_value, m_p_value), m_p_begin_value,
                      m_p_value, mid_pointer(p_begin_metadata, m_p_metadata)));
  }

  /// Returns the node iterator associated with the right node.
  inline this_type get_r_child() const {
    ...
    return (this_type(
        mid_pointer(m_p_value + 1, m_p_end_value), m_p_value + 1, m_p_end_value,
        (m_p_metadata == 0) ? 0
  ...
  inline bool operator==(const this_type& other) const {
    ...
    if (is_end) return (is_other_end);

    if (is_other_end) return (is_end);
...
class ov_tree_node_it_ : public PB_DS_OV_TREE_CONST_NODE_ITERATOR_C_DEC {
 ...
 public:
  ...
  typedef typename rebind_traits<_Alloc, Value_Type>::pointer value_type;

  typedef typename rebind_traits<
      _Alloc, typename remove_const<Value_Type>::type>::pointer reference;

  typedef typename rebind_traits<
      _Alloc, typename remove_const<Value_Type>::type>::pointer const_reference;

  inline ov_tree_node_it_(const_pointer p_nd = 0, const_pointer p_begin_nd = 0,
                          const_pointer p_end_nd = 0,
                          const_metadata_pointer p_metadata = 0)
      : base_type(p_nd, p_begin_nd, p_end_nd, p_metadata) {}

  /// Access.
  inline reference operator*() const { return reference(base_type::m_p_value); }

  /// Returns the node reference associated with the left node.
  inline ov_tree_node_it_ get_l_child() const {
    if (base_type::m_p_begin_value == base_type::m_p_value)
      return (this_type(base_type::m_p_begin_value, base_type::m_p_begin_value,
  ...
  inline ov_tree_node_it_ get_r_child() const {
    ...
    return (this_type(base_type::mid_pointer(base_type::m_p_value + 1,
                                             base_type::m_p_end_value),
                      base_type::m_p_value + 1, base_type::m_p_end_value,
                      (base_type::m_p_metadata == 0)
                          ? 0
                          : base_type::mid_pointer(base_type::m_p_metadata + 1,
                                                   p_end_metadata)));
  }
};
