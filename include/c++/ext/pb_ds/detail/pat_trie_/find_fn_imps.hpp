...
PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::point_iterator PB_DS_CLASS_C_DEC::find(
  ...
  if (synth_access_traits::equal_keys(
...
PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::point_const_iterator PB_DS_CLASS_C_DEC::find(
...
PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::node_pointer PB_DS_CLASS_C_DEC::find_imp(
    key_const_reference r_key) {
  if (empty()) return 0;
  ...
  while (p_nd->m_type != leaf_node) {
    ...
    node_pointer p_next_nd =
        static_cast<inode_pointer>(p_nd)->get_child_node(b_it, e_it, this);

    if (p_next_nd == 0) return p_nd;
    p_nd = p_next_nd;
  }
  return p_nd;
}
...
PB_DS_CLASS_C_DEC::lower_bound_imp(key_const_reference r_key) {
  if (empty()) return (m_p_head);
  ...
  while (true) {
    ...
    p_nd = static_cast<inode_pointer>(p_nd)->get_lower_bound_child_node(
        b_it, e_it, checked_ind, this);
...
PB_DS_CLASS_C_DEC::lower_bound(key_const_reference r_key) {
  return point_iterator(lower_bound_imp(r_key));
}

PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::point_const_iterator
PB_DS_CLASS_C_DEC::lower_bound(key_const_reference r_key) const {
  return point_const_iterator(
      const_cast<PB_DS_CLASS_C_DEC*>(this)->lower_bound_imp(r_key));
}

PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::point_iterator
PB_DS_CLASS_C_DEC::upper_bound(key_const_reference r_key) {
  point_iterator l_bound_it = lower_bound(r_key);

  _GLIBCXX_DEBUG_ASSERT(
      l_bound_it == end() ||
      !synth_access_traits::cmp_keys(PB_DS_V2F(*l_bound_it), r_key));
...
PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::point_const_iterator
PB_DS_CLASS_C_DEC::upper_bound(key_const_reference r_key) const {
...
PB_DS_CLASS_C_DEC::pref_begin(node_const_pointer p_nd) {
  if (p_nd->m_type == leaf_node)
    return (synth_access_traits::begin(
...
PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::a_const_iterator PB_DS_CLASS_C_DEC::pref_end(
    node_const_pointer p_nd) {
  if (p_nd->m_type == leaf_node)
    return (synth_access_traits::end(
        PB_DS_V2F(static_cast<leaf_const_pointer>(p_nd)->value())));
...
PB_DS_CLASS_C_DEC::leftmost_descendant(node_const_pointer p_nd) {
  if (p_nd->m_type == leaf_node) return static_cast<leaf_const_pointer>(p_nd);
  return static_cast<inode_const_pointer>(p_nd)->leftmost_descendant();
}

PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::leaf_pointer
PB_DS_CLASS_C_DEC::leftmost_descendant(node_pointer p_nd) {
...
PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::leaf_const_pointer
PB_DS_CLASS_C_DEC::rightmost_descendant(node_const_pointer p_nd) {
  if (p_nd->m_type == leaf_node) return static_cast<leaf_const_pointer>(p_nd);
...
PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::leaf_pointer
PB_DS_CLASS_C_DEC::rightmost_descendant(node_pointer p_nd) {
  if (p_nd->m_type == leaf_node) return static_cast<leaf_pointer>(p_nd);
