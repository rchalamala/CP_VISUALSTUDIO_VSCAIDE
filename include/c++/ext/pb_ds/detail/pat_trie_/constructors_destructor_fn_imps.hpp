...
PB_DS_CLASS_T_DEC
typename PB_DS_CLASS_C_DEC::head_allocator PB_DS_CLASS_C_DEC::s_head_allocator;
...
PB_DS_CLASS_T_DEC
PB_DS_CLASS_C_DEC::PB_DS_PAT_TRIE_NAME()
    : m_p_head(s_head_allocator.allocate(1)), m_size(0) {
  initialize();
  PB_DS_ASSERT_VALID((*this))
}

PB_DS_CLASS_T_DEC
PB_DS_CLASS_C_DEC::PB_DS_PAT_TRIE_NAME(const access_traits& r_access_traits)
...
PB_DS_CLASS_T_DEC
PB_DS_CLASS_C_DEC::PB_DS_PAT_TRIE_NAME(const PB_DS_CLASS_C_DEC& other)
    :
...
void PB_DS_CLASS_C_DEC::copy_from_range(It first_it, It last_it) {
  while (first_it != last_it) insert(*(first_it++));
}

PB_DS_CLASS_T_DEC
typename PB_DS_CLASS_C_DEC::node_pointer PB_DS_CLASS_C_DEC::recursive_copy_node(
    node_const_pointer p_ncp) {
  ...
  __catch(...) {
    while (child_i-- > 0) clear_imp(a_p_children[child_i]);
    __throw_exception_again;
  }
