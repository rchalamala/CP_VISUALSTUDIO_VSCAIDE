...
#define PB_DS_B_HEAP_BASE                                                      \
  left_child_next_sibling_heap<Value_Type, Cmp_Fn, typename _Alloc::size_type, \
                               _Alloc, false>
#else
#define PB_DS_B_HEAP_BASE                                                      \
  left_child_next_sibling_heap<Value_Type, Cmp_Fn, typename _Alloc::size_type, \
...
template <typename Value_Type, typename Cmp_Fn, typename _Alloc>
class binomial_heap_base : public PB_DS_B_HEAP_BASE {
 private:
...
#ifdef _GLIBCXX_DEBUG
  void assert_valid(bool, const char*, int) const;

  void assert_max(const char*, int) const;
#endif

 private:
  inline node_pointer fix(node_pointer) const;

  inline void insert_node(node_pointer);

  inline void remove_parentless_node(node_pointer);

  inline node_pointer join(node_pointer, node_pointer) const;

#ifdef _GLIBCXX_DEBUG
  void assert_node_consistent(node_const_pointer, bool, bool, const char*,
                              int) const;
#endif
...
#define PB_DS_ASSERT_BASE_NODE_CONSISTENT(_Node, _Bool) \
  _GLIBCXX_DEBUG_ONLY(                                  \
      base_type::assert_node_consistent(_Node, _Bool, __FILE__, __LINE__);)
