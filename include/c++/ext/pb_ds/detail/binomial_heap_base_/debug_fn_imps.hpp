...
PB_DS_CLASS_T_DEC
void PB_DS_CLASS_C_DEC::assert_valid(bool strictly_binomial, const char* __file,
                                     int __line) const {
  base_type::assert_valid(__file, __line);
  assert_node_consistent(base_type::m_p_root, strictly_binomial, true, __file,
                         __line);
...
void PB_DS_CLASS_C_DEC::assert_node_consistent(node_const_pointer p_nd,
  ...
  if (p_nd->m_p_next_sibling != 0) {
    if (increasing) {
      ...
      PB_DS_DEBUG_VERIFY(p_nd->m_metadata > p_nd->m_p_next_sibling->m_metadata);
  }
}
