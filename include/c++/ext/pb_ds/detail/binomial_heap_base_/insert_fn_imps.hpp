...
PB_DS_CLASS_T_DEC
inline void PB_DS_CLASS_C_DEC::insert_node(node_pointer p_nd) {
...
PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::node_pointer PB_DS_CLASS_C_DEC::fix(
    node_pointer p_nd) const {
  while (p_nd->m_p_next_sibling != 0 &&
    ...
    if (Cmp_Fn::operator()(p_nd->m_value, p_next->m_value)) {
      ...
      if (p_nd->m_p_next_sibling != 0) p_next->m_p_next_sibling = 0;
...
void PB_DS_CLASS_C_DEC::modify(point_iterator it, const_reference r_new_val) {
  ...
  if (bubble_up) {
    ...
    if (p_nd->m_p_prev_or_parent == 0) base_type::m_p_root = p_nd;
