...
void PB_DS_CLASS_C_DEC::split(Pred pred, PB_DS_CLASS_C_DEC& other) {
  ...
  while (p_out != 0) {
    ...
    if (other.m_p_root != 0) other.m_p_root->m_p_prev_or_parent = p_out;
...
PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::node_pointer PB_DS_CLASS_C_DEC::join(
    node_pointer p_lhs, node_pointer p_rhs) const {
  ...
  if (p_cur != 0) p_cur->m_p_next_sibling = 0;

  if (p_ret != 0) p_ret->m_p_prev_or_parent = 0;
