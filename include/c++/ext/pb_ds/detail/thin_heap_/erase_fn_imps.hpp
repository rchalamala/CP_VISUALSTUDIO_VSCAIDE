...
PB_DS_CLASS_T_DEC
inline void PB_DS_CLASS_C_DEC::remove_max_node() {
  to_aux_except_max();
  make_from_aux();
}

PB_DS_CLASS_T_DEC
void PB_DS_CLASS_C_DEC::to_aux_except_max() {
  ...
  while (p_add != 0) {
    node_pointer p_next_add = p_add->m_p_next_sibling;
    p_add->m_metadata =
        p_add->m_p_l_child == 0 ? 0 : p_add->m_p_l_child->m_metadata + 1;
...
PB_DS_CLASS_T_DEC
inline void PB_DS_CLASS_C_DEC::add_to_aux(node_pointer p_nd) {
...
PB_DS_CLASS_T_DEC
inline void PB_DS_CLASS_C_DEC::make_from_aux() {
...
PB_DS_CLASS_T_DEC
inline void PB_DS_CLASS_C_DEC::remove_node(node_pointer p_nd) {
  ...
  if (p_fix != 0) p_fix->m_p_next_sibling = p_nd;

  remove_max_node();
}

PB_DS_CLASS_T_DEC
inline void PB_DS_CLASS_C_DEC::clear() {
  base_type::clear();
  m_p_max = 0;
}
...
PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::size_type PB_DS_CLASS_C_DEC::rank_bound() {
  using namespace std;
  const size_t* const p_upper = std::upper_bound(
      g_a_rank_bounds, g_a_rank_bounds + num_distinct_rank_bounds,
      base_type::m_size);

  if (p_upper == g_a_rank_bounds + num_distinct_rank_bounds) return max_rank;
