...
PB_DS_CLASS_T_DEC
void PB_DS_CLASS_C_DEC::assert_aux_null(const char* __file, int __line) const {
  for (size_type i = 0; i < max_rank; ++i) PB_DS_DEBUG_VERIFY(m_a_aux[i] == 0);
}
...
void PB_DS_CLASS_C_DEC::assert_max(const char* __file, int __line) const {
  ...
  for (const_iterator it = base_type::begin(); it != base_type::end(); ++it)
    PB_DS_DEBUG_VERIFY(
        !Cmp_Fn::operator()(m_p_max->m_value, it.m_p_nd->m_value));
}

PB_DS_CLASS_T_DEC
void PB_DS_CLASS_C_DEC::assert_node_consistent(node_const_pointer p_nd,
                                               bool root, const char* __file,
                                               int __line) const {
  ...
  if (!root) {
    if (p_nd->m_metadata == 0)
      ...
      PB_DS_DEBUG_VERIFY(p_nd->m_metadata ==
                         p_nd->m_p_next_sibling->m_metadata + 1);
  }

  if (p_nd->m_p_l_child != 0)
    PB_DS_DEBUG_VERIFY(p_nd->m_p_l_child->m_metadata + 1 ==
                       base_type::degree(p_nd));
  ...
  if (root) PB_DS_DEBUG_VERIFY(unmarked_valid);
}
