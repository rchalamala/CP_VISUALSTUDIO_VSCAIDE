...
PB_DS_CLASS_T_DEC
inline typename PB_DS_CLASS_C_DEC::comp_hash PB_DS_CLASS_C_DEC::find_ins_pos(
    key_const_reference r_key, true_type) {
  ...
  for (i = 0; i < m_num_e; ++i) {
    ...
    switch (p_e->m_stat) {
      ...
      case erased_entry_status:
        if (ins_pos == m_num_e) ins_pos = pos;
        break;
  ...
  resize_base::notify_insert_search_end();
  if (ins_pos == m_num_e) __throw_insert_error();
  return std::make_pair(ins_pos, pos_hash_pair.second);
}

PB_DS_CLASS_T_DEC
inline std::pair<typename PB_DS_CLASS_C_DEC::point_iterator, bool>
PB_DS_CLASS_C_DEC::insert_imp(const_reference r_val, true_type) {
