...
struct trie_string_access_traits {
 public:
  ...
  typedef typename detail::__conditional_type<
      Reverse, typename String::const_reverse_iterator,
  ...
  inline static const_iterator begin(key_const_reference);

  /// Returns a const_iterator to the after-last element of
  /// key_const_reference argument.
  inline static const_iterator end(key_const_reference);

  /// Maps an element to a position.
  inline static size_type e_pos(e_type e);

 private:
  inline static const_iterator begin_imp(key_const_reference,
                                         detail::false_type);

  inline static const_iterator begin_imp(key_const_reference,
                                         detail::true_type);

  inline static const_iterator end_imp(key_const_reference, detail::false_type);

  inline static const_iterator end_imp(key_const_reference, detail::true_type);
...
#define PB_DS_CLASS_C_DEC \
  trie_prefix_search_node_update<Node_CItr, Node_Itr, _ATraits, _Alloc>
...
class trie_prefix_search_node_update : private PB_DS_TRIE_POLICY_BASE {
 ...
 public:
  ...
  std::pair<const_iterator, const_iterator> prefix_range(
  ...
  /// Finds the iterator range corresponding to all values whose
  /// prefixes match [b, e).
  std::pair<iterator, iterator> prefix_range(a_const_iterator,
                                             a_const_iterator);

 protected:
  /// Called to update a node's metadata.
  inline void operator()(node_iterator node_it,
                         node_const_iterator end_nd_it) const;

 private:
  node_iterator next_child(node_iterator, a_const_iterator, a_const_iterator,
...
#define PB_DS_CLASS_C_DEC \
  trie_order_statistics_node_update<Node_CItr, Node_Itr, _ATraits, _Alloc>
...
class trie_order_statistics_node_update : private PB_DS_TRIE_POLICY_BASE {
 ...
 public:
  ...
  inline size_type order_of_prefix(a_const_iterator, a_const_iterator) const;
 ...
 private:
  ...
  _GLIBCXX_NODISCARD virtual bool empty() const = 0;
