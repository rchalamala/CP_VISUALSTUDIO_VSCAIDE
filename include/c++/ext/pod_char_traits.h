...
namespace __gnu_cxx _GLIBCXX_VISIBILITY(default) {
  ...
  inline bool operator==(const character<_Value, _Int, _St>& lhs,
  ...
  struct char_traits<__gnu_cxx::character<_Value, _Int, _St> > {
    ...
    static int compare(const char_type* __s1, const char_type* __s2,
      ...
      for (size_t __i = 0; __i < __n; ++__i)
        if (!eq(__s1[__i], __s2[__i])) return lt(__s1[__i], __s2[__i]) ? -1 : 1;
      return 0;
    }
    ...
    static int_type eof() {
      int_type __r = {static_cast<typename __gnu_cxx::__conditional_type<
          std::__is_integer<int_type>::__value, int_type, int>::__type>(-1)};
      return __r;
    }
