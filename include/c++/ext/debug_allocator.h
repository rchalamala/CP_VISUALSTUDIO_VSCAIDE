...
namespace __gnu_cxx _GLIBCXX_VISIBILITY(default) {
  ...
  class debug_allocator {
   ...
   private:
    ...
    template <typename _Alloc2, typename = typename __alloc_traits<_Alloc2>::
   ...
   public:
    ...
    _GLIBCXX_NODISCARD pointer allocate(size_type __n, const void* __hint) {
    ...
    void construct(_Tp* __p, _Args&&... __args) {
      _Traits::construct(_M_allocator, __p, std::forward<_Args>(__args)...);
    }
#endif
    ...
    template <typename _Alloc2>
    friend bool operator==(const debug_allocator& __lhs,
