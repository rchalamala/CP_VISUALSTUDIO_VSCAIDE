...
namespace __gnu_cxx _GLIBCXX_VISIBILITY(default) {
  ...
  class malloc_allocator {
   public:
    ...
    template <typename _Tp1>
    _GLIBCXX20_CONSTEXPR malloc_allocator(const malloc_allocator<_Tp1>&)
    ...
    _Tp* allocate(size_type __n, const void* = 0) {
      ...
      if (alignof(_Tp) > alignof(std::max_align_t)) {
        __ret =
            static_cast<_Tp*>(::aligned_alloc(alignof(_Tp), __n * sizeof(_Tp)));
      }
#else
#define _GLIBCXX_CHECK_MALLOC_RESULT
#endif
#endif
      if (!__ret) __ret = static_cast<_Tp*>(std::malloc(__n * sizeof(_Tp)));
      if (!__ret) std::__throw_bad_alloc();
...
#undef _GLIBCXX_CHECK_MALLOC_RESULT
    ...
    void deallocate(_Tp* __p, size_type) { std::free(static_cast<void*>(__p)); }
    ...
    template <typename _Up>
    friend _GLIBCXX20_CONSTEXPR bool operator==(const malloc_allocator&,
