...
namespace __gnu_cxx _GLIBCXX_VISIBILITY(default) {
   ...
   public:
    ...
    inline void check(size_t label) {
      std::string found;
      {
        ...
        while (beg != end) {
          if (beg->second.first == label) log_to_string(found, *beg);
          ++beg;
        }
      }
...
#if __cplusplus >= 201103L
      {
        ...
        while (beg != end) {
          if (beg->second == label) log_to_string(found, *beg);
          ++beg;
        }
      }
#endif
...
#if __cplusplus >= 201103L
    static void log_to_string(std::string& s,
                              const std::pair<const void*, size_t>& ref) {
      char buf[40];
      const char tab('\t');
      s += "label: ";
  ...
  struct limit_condition : public condition_base {
    ...
    static void throw_conditionally() {
      if (count() == limit()) __throw_forced_error();
      ++count();
    }
  ...
  struct random_condition : public condition_base {
    ...
    struct group_adjustor : public adjustor_base {
      group_adjustor(size_t size) {
        set_probability(
            1 - std::pow(double(1 - probability()), double(0.5 / (size + 1))));
      }
    };
    ...
    static void throw_conditionally() {
      if (generate() < probability()) __throw_forced_error();
    }
  ...
  template <typename _Cond>
  inline void swap(throw_value_base<_Cond> & __a,
  ...
  template <typename _Cond>
  inline bool operator==(const throw_value_base<_Cond>& __a,
  ...
  template <typename _Cond>
  inline throw_value_base<_Cond> operator+(const throw_value_base<_Cond>& __a,
                                           const throw_value_base<_Cond>& __b) {
    typedef throw_value_base<_Cond> throw_value;
    throw_value::throw_conditionally();
  ...
  class throw_allocator_base : public annotate_base, public _Cond {
   ...
   public:
    ...
    _GLIBCXX_NODISCARD pointer allocate(size_type __n, const void* hint = 0) {
