...
namespace __gnu_cxx _GLIBCXX_VISIBILITY(default) {
  ...
  class _Std_pointer_impl {
   public:
    ...
    inline _Tp* get() const { return _M_value; }

    // A method to set the pointer value, from a standard T* value;
    inline void set(element_type* __arg) { _M_value = __arg; }

    // Comparison of pointers
    inline bool operator<(const _Std_pointer_impl& __rarg) const {
  ...
  class _Relative_pointer_impl {
   public:
    ...
    // Comparison of pointers
    inline bool operator<(const _Relative_pointer_impl& __rarg) const {
  ...
  class _Pointer_adapter : public _Storage_policy {
   public:
    ...
    _Pointer_adapter(element_type* __arg = 0) { _Storage_policy::set(__arg); }
    ...
    inline reference operator*() const { return *(_Storage_policy::get()); }

    // Operator->, returns element_type*
    inline element_type* operator->() const { return _Storage_policy::get(); }

    // Operator[], returns a element_type& to the item at that loc.
    inline reference operator[](std::ptrdiff_t __index) const {
   ...
   public:
    operator __unspecified_bool_type() const {
      return _Storage_policy::get() == 0 ? 0 : &_Pointer_adapter::operator->;
    }

    // ! operator (for: if (!ptr)...)
    inline bool operator!() const { return (_Storage_policy::get() == 0); }
#endif

    // Pointer differences
    inline friend std::ptrdiff_t operator-(const _Pointer_adapter& __lhs,
                                           element_type* __rhs) {
      return (__lhs.get() - __rhs);
    }

    inline friend std::ptrdiff_t operator-(element_type* __lhs,
    ...
    template <typename _Up>
    inline friend std::ptrdiff_t operator-(_Up* __lhs,
                                           const _Pointer_adapter& __rhs) {
      return (__lhs - __rhs.get());
    }

    template <typename _Up>
    inline std::ptrdiff_t operator-(const _Pointer_adapter<_Up>& __rhs) const {
...
#define _CXX_POINTER_ARITH_OPERATOR_SET(INT_TYPE)                           \
  inline friend _Pointer_adapter operator+(const _Pointer_adapter& __lhs,   \
                                           INT_TYPE __offset) {             \
    return _Pointer_adapter(__lhs.get() + __offset);                        \
  }                                                                         \
                                                                            \
  inline friend _Pointer_adapter operator+(INT_TYPE __offset,               \
                                           const _Pointer_adapter& __rhs) { \
  ...
  inline friend _Pointer_adapter operator-(const _Pointer_adapter& __lhs,   \
    ...
    return _Pointer_adapter(__lhs.get() - __offset);                        \
                                                                            ...
                                                                            \
  inline _Pointer_adapter& operator+=(INT_TYPE __offset) {                  \
  ...
  // END of _CXX_POINTER_ARITH_OPERATOR_SET macro
    ...
    inline _Pointer_adapter& operator++() {
      _Storage_policy::set(_Storage_policy::get() + 1);
      return *this;
    }

    inline _Pointer_adapter operator++(int) {
    ...
    friend std::strong_ordering operator<=>(
        const _Pointer_adapter& __lhs, const _Pointer_adapter& __rhs) noexcept {
...
#define _GCC_CXX_POINTER_COMPARISON_OPERATION_SET(OPERATOR)            \
  ...
  inline bool operator OPERATOR(const _Pointer_adapter<_Tp1>& __lhs,   \
                                _Tp2 __rhs) {                          \
    return __lhs.get() OPERATOR __rhs;                                 \
  }                                                                    \
                                                                       \
  template <typename _Tp1, typename _Tp2>                              \
  inline bool operator OPERATOR(_Tp1 __lhs,                            \
  ...
  template <typename _Tp>
  inline bool operator==(const _Pointer_adapter<_Tp>& __lhs, int __rhs) {
    return __lhs.get() == reinterpret_cast<void*>(__rhs);
  }

  template <typename _Tp>
  inline bool operator==(int __lhs, const _Pointer_adapter<_Tp>& __rhs) {
   ...
   */
  template <typename _Tp>
  inline bool operator==(const _Pointer_adapter<_Tp>& __lhs,
