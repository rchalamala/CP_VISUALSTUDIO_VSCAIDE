...
namespace __gnu_cxx _GLIBCXX_VISIBILITY(default) {
  ...
  class __mini_vector {
   ...
   public:
    ...
    __mini_vector() : _M_start(0), _M_finish(0), _M_end_of_storage(0) {}

    size_type size() const throw() { return _M_finish - _M_start; }

    iterator begin() const throw() { return this->_M_start; }

    iterator end() const throw() { return this->_M_finish; }

    reference back() const throw() { return *(this->end() - 1); }
    ...
    void pop_back() throw() { --this->_M_finish; }

    void erase(iterator __pos) throw();

    void clear() throw() { this->_M_finish = this->_M_start; }
  };

  // Out of line function definitions.
  template <typename _Tp>
  void __mini_vector<_Tp>::insert(iterator __pos, const_reference __x) {
    if (this->_M_space_left()) {
      ...
      *__start = __x;
      ...
      if (this->_M_start) this->deallocate(this->_M_start, this->size());
  ...
  template <typename _Tp>
  void __mini_vector<_Tp>::erase(iterator __pos) throw() {
  ...
  class _Ffit_finder
   ...
   public:
    ...
    bool operator()(_Block_pair __bp) throw() {
      ...
      if (*(reinterpret_cast<size_t*>(__bp.first) - (__diff + 1)) ==
          __detail::__num_blocks(__bp))
        return false;
    ...
    std::size_t* _M_get() const throw() { return _M_pbitmap; }
  ...
  class _Bitmap_counter {
   ...
   public:
    ...
    void _M_reset(long __index = -1) throw() {
      ...
      _M_curr_bmap =
          reinterpret_cast<std::size_t*>(_M_vbp[_M_curr_index].first) - 1;

      _GLIBCXX_DEBUG_ASSERT(__index <= (long)_M_vbp.size() - 1);

      _M_last_bmap_in_block =
          _M_curr_bmap -
          ((_M_vbp[_M_curr_index].second - _M_vbp[_M_curr_index].first + 1) /
    ...
    bool _M_finished() const throw() { return (_M_curr_bmap == 0); }
    ...
    std::size_t* _M_get() const throw() { return _M_curr_bmap; }

    pointer _M_base() const throw() { return _M_vbp[_M_curr_index].first; }

    _Index_type _M_offset() const throw() {
      return std::size_t(bits_per_block) *
             ((reinterpret_cast<std::size_t*>(this->_M_base()) - _M_curr_bmap) -
              1);
    }
  ...
  class free_list {
   ...
   private:
    ...
    void _M_validate(std::size_t* __addr) throw() {
      ...
      iterator __temp =
          __detail::__lower_bound(__free_list.begin(), __free_list.end(),
     ...
     *  false.
     */
    bool _M_should_i_give(std::size_t __block_size,
  ...
  class bitmap_allocator : private free_list {
   ...
   private:
    ...
    static _BPiter _S_find(_Predicate __p) {
      ...
      while (__first != _S_mem_blocks.end() && !__p(*__first)) ++__first;
      return __first;
    }
    ...
    void _S_refill_pool() _GLIBCXX_THROW(std::bad_alloc) {
      ...
      const size_t __num_bitmaps =
          (_S_block_size / size_t(__detail::bits_per_block));
      ...
      *__temp = 0;
      ...
      _Block_pair __bp = std::make_pair(
   ...
   public:
    ...
    void _M_deallocate_single_object(pointer __p) throw() {
      ...
      const size_t __rotate =
          (__displacement % size_t(__detail::bits_per_block));
      size_t* __bitmapC =
          reinterpret_cast<size_t*>(_S_mem_blocks[__diff].first) - 1;
      __bitmapC -= (__displacement / size_t(__detail::bits_per_block));

      __detail::__bit_free(__bitmapC, __rotate);
      size_t* __puse_count =
          reinterpret_cast<size_t*>(_S_mem_blocks[__diff].first) -
   ...
   public:
    ...
    pointer address(reference __r) const _GLIBCXX_NOEXCEPT {
      return std::__addressof(__r);
    }

    const_pointer address(const_reference __r) const _GLIBCXX_NOEXCEPT {
      return std::__addressof(__r);
    }
  ...
  template <typename _Tp>
  __detail::_Bitmap_counter<typename bitmap_allocator<_Tp>::_Alloc_block*>
      bitmap_allocator<_Tp>::_S_last_request(_S_mem_blocks);

#if defined __GTHREADS
  template <typename _Tp>
  typename bitmap_allocator<_Tp>::__mutex_type bitmap_allocator<_Tp>::_S_mut;
#endif
