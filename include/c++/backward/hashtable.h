...
namespace __gnu_cxx _GLIBCXX_VISIBILITY(default) {
  ...
  inline unsigned long __stl_next_prime(unsigned long __n) {
    const unsigned long* __first =
        _Hashtable_prime_list<unsigned long>::_S_get_prime_list();
  ...
  class hashtable {
   ...
   private:
    ...
    typedef typename _Alloc_traits::template rebind<_Node>::other _Node_Alloc;
    typedef
        typename _Alloc_traits::template rebind<_Node*>::other _Nodeptr_Alloc;
   ...
   public:
    ...
    iterator begin() {
      for (size_type __n = 0; __n < _M_buckets.size(); ++__n)
        if (_M_buckets[__n]) return iterator(_M_buckets[__n], this);
      return end();
    }
    ...
    const_iterator begin() const {
      for (size_type __n = 0; __n < _M_buckets.size(); ++__n)
        if (_M_buckets[__n]) return const_iterator(_M_buckets[__n], this);
      return end();
    }
   ...
   public:
    ...
    size_type max_bucket_count() const {
      return _Hashtable_prime_list<
          unsigned long>::_S_get_prime_list()[(int)_S_num_primes - 1];
    }
  ...
  bool operator==(const hashtable<_Val, _Key, _HF, _Ex, _Eq, _All>& __ht1,
    ...
    if (__ht1._M_buckets.size() != __ht2._M_buckets.size()) return false;
    ...
    for (std::size_t __n = 0; __n < __ht1._M_buckets.size(); ++__n) {
      ...
      if (__cur1 || __cur2) return false;
      // Now check one's elements are in the other
      for (__cur1 = __ht1._M_buckets[__n]; __cur1; __cur1 = __cur1->_M_next) {
        bool _found__cur1 = false;
        for (__cur2 = __ht2._M_buckets[__n]; __cur2; __cur2 = __cur2->_M_next) {
          if (__cur1->_M_val == __cur2->_M_val) {
            _found__cur1 = true;
            break;
          }
        }
        if (!_found__cur1) return false;
      }
    }
    return true;
  }
